...
### Docs view
This plan IS the docs task — every new/renamed public symbol gets an API-reference page matching its export name. Per `docs.md` §Function & Prefix Docs (Function template) and §File Locations & Naming (lowercase, matches export name). The existing `packages/dom/docs/api/mount.mdx` is NOT in scope here — its `MountHandle` modification rides with `plans/dom/code/dom-reset-mount-handle.md`'s trio Docs task (the surface change that changed `mount()`'s return type owns its own existing page).
...
- [ ] No claim contradicts the implementation — each reset's stated cleared-state cross-checked against its source; `flush` cross-checked against `@hellajs/core`'s barrel
