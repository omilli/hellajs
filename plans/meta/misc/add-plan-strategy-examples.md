...
- [ ] IF the operator's global `memory/` is present in this environment: `memory/entries/plan-accessibility.md` updated to "addressed" with a pointer to the new guidance (or its INDEX line refined). Otherwise no-op — `memory/` is operator-side, not vendored in this repo (AGENTS.md §Skills Graceful degradation); the memory write is the operator's to make, not a gate on this plan
