...
**Object and Array Updates**: Signals use strict equality (`===`) to detect changes. Always create new references for objects and arrays.
...
**JSX Integration**: Always pass the signal function to JSX, never call it directly.

```jsx
// ✅ Pass signal function
<div>{count}</div>
// ❌ Calling breaks reactivity
<div>{count()}</div>
```

**TypeScript Safety**: Initialize signals with proper default values to avoid runtime errors.
...
**Form State Management**: Track individual form field values with automatic validation triggers:

```typescript
const form = {
  email: signal(''),
  password: signal(''),
  isValid: computed(() => form.email().includes('@') && form.password().length > 8)
};
```

**UI State Coordination**: Manage related UI elements that need to stay synchronized:

```typescript
const sidebar = signal(false);
const modal = signal(false);

// Close sidebar when modal opens
effect(() => {
  if (modal()) sidebar(false);
});
```

**Data Transformation Pipelines**: Create chains of derived data that automatically update:

```typescript
const rawData = signal([]);
const filteredData = computed(() => rawData().filter(item => item.active));
const sortedData = computed(() => [...filteredData()].sort((a, b) => a.name.localeCompare(b.name)));
```

**Real-time Data Binding**: Connect signals to WebSocket or EventSource streams:

```typescript
const livePrice = signal(0);
const ws = new WebSocket('wss://api.example.com/prices');
ws.onmessage = (event) => livePrice(JSON.parse(event.data).price);
```
