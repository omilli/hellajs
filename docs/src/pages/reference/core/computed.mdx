...
```typescript
function computed<T>(computeFn: (previousValue?: T) => T): () => T
```
- `computeFn`: A function that computes the value. It receives the previously computed value as an optional argument.
...
### Previous Value
The `computeFn` function receives the previously computed value as its first argument, enabling incremental calculations and optimizations.
...
The `computeFn` function should not have side effects. Use [`effect`](/reference/core/effect) for side effects.
