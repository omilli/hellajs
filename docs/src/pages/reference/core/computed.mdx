...
**Keep Getters Pure**: The `getter` function should not have side effects. Use [`effect`](/reference/core/effect) for side effects.
...
**Avoid Complex Logic**: Break complex computations into smaller, chained computed signals for better performance and debugging.
...
**Data Aggregation**: Calculate totals, averages, and statistics from reactive collections:

```typescript
const orders = signal([]);
const revenue = computed(() => orders().reduce((sum, order) => sum + order.total, 0));
const averageOrderValue = computed(() => revenue() / orders().length || 0);
```

**Complex Validations**: Build sophisticated validation logic that adapts to form state:

```typescript
const user = signal({ email: '', age: 0, terms: false });
const isValidUser = computed(() => {
  const u = user();
  return u.email.includes('@') && u.age >= 18 && u.terms;
});
```

**UI State Derivation**: Derive display states from multiple reactive sources:
...
const displayName = computed(() => {
  ...
  return user ? `${user.firstName} (${theme()} mode)` : 'Guest';
});
```

**Performance Optimization**: Cache expensive calculations that depend on reactive data:

```typescript
const dataset = signal([]);
const expensiveAnalysis = computed(() => {
  return performComplexAnalysis(dataset()); // Only runs when dataset changes
});
```
