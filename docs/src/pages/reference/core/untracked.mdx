...
**Dependency Isolation**: Creates a reactive boundary where signals can be read without establishing dependency relationships with the calling context.
...
**Selective Tracking**: Enables fine-grained control over which signals trigger reactive updates, preventing unnecessary recalculations.
...
**Debug-Safe Reading**: Essential for debug logging and conditional features that shouldn't influence reactive behavior.
...
**Performance Optimization**: Prevents unwanted dependencies that could cause expensive computations to run more frequently than necessary.
...
**Overusing `untracked`**: Most of the time, you want dependencies to be tracked automatically. Only use `untracked` when you have a specific reason to prevent a dependency.
...
**Debug Code Dependencies**: Always wrap debug-only signal reads in `untracked` to avoid unwanted dependencies.
