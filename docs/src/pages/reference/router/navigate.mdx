...
**Asynchronous Navigation**: Navigation is asynchronous - the route change isn't immediate.
...
**Replace vs Push**: Using `replace: true` prevents users from navigating back to the previous page.
...
**Parameter Validation**: Invalid parameter keys (not matching route pattern) are ignored.
