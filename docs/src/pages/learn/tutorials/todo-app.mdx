...
Update your `vite.config.ts` to use the HellaJS plugin and Tailwind CSS.
...
Import this CSS file in your `main.ts`.
...
Replace the content of `main.ts` with the basic todo app.
      ...
      <ul class="space-y-2">
        <ForEach each={todos} use={(todo) => (
          <li key={todo.id} class="flex items-center gap-2 p-2 border rounded">
            ...
            />
            <span bind:class={() => `flex-1 ${todo.completed ? 'line-through text-gray-500' : ''}`}>
              {todo.text}
            </span>
            <button
...
- **`bind:value={newTodoText}`** - Creates reactive two-way binding for the input field
- **`<ForEach each={todos} use={(todo) => ...} />`** - Renders the todo list with optimized list diffing
- **Array immutability** - Using spread operator `[...todos(), newTodo]` for reactive updates
- **Event handlers** - `on:click`, `on:input`, and `on:keydown` for user interactions
...
Add filter buttons between the input and the list in the JSX return:
...
<div class="flex gap-2 mb-4">
  <button
    bind:class={() => `px-3 py-1 rounded text-sm ${
      filter() === 'all'
...
- **Reactive button styling** - `bind:class` changes button appearance based on active filter
...
Add a `clearCompleted` function after `removeTodo`:
...
Update the header and add a "Clear Done" button in the JSX:
...
<details>
<summary>Complete Final Code</summary>
