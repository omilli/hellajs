...
Replace the content of `main.ts` with the component shell.
...
const TodoApp = () => {
  ...
  // Styles and actions coming next
};
...
```tsx
// ... previous code unchanged, adding after imports:
...
- **[`css`](/reference/css/css)`({ body: {...} }, { global: true })`** - Resets body styles globally
- **`const app = css({...})`** - Returns a class name with nested `h1` and `button` rules
- **`const list = css({ ..., 'li': {...} })`** - Scopes list item styles inside the list
- **`const flex = css({ flex: '1' })`** - Reusable utility class for expanding elements
- **`const done = css({...})`** - Style for completed todo text
...
```tsx
// Inside TodoApp, after the signals:
...
```tsx
// Inside TodoApp, after the actions:
    ...
    <ul class={list}>
      <ForEach each={todos} use={(todo) => (
        <li key={todo.id}>
          ...
          />
          <span bind:class={() => [flex, todo.completed && done]}>
            {todo.text}
          </span>
...
- **`bind:value={newTodoText}`** - Two-way binding for the input field
...
- **`bind:class={() => [flex, todo.completed && done]}`** - Conditionally adds the `done` class when completed
...
```tsx
// ... previous code unchanged, adding:
...
// Inside TodoApp, add a filter signal:
...
```tsx
// ... inside the return, after the input div, before <ul>:
...
Also add a third initial todo so the completed filter has data to show:
...
Add the editing functions after `removeTodo`:
...
```tsx
// ... previous code unchanged, updating the <ForEach> item:
<ForEach each={filteredTodos} use={(todo) => (
  <li key={todo.id}>
    <input
      type="checkbox"
...
```tsx
// ... previous code unchanged, adding before TodoApp:
