...
Define styles using [`css`](/reference/css/css). Use string class selectors like `'.app'` inside a single `css()` call.
...
**Code Explanation**
- **`css({ body: {...}, '.app': {...}, ... })`**:  A single `css()` call with all styles — global selectors (`body`) and class selectors (`'.app'`) together
- **`'.app'`**:  String class selectors create styles scoped to that class name, used in HTML as `class="app"`
