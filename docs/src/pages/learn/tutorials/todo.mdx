...
Define styles using [`css`](/reference/css/css). Use `{ name: 'x' }` to create a scoped class that returns `'x'` for class attributes.
...
**Code Explanation**
- **`css({ body: {...} })`**:  Without `name`, styles are applied globally (returns `''`)
- **`css({...}, { name: 'app' })`**:  Creates scoped styles and returns `'app'` for use in class attributes
- **`li`**:  Nested CSS selectors work as expected
