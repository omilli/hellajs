...
Replace the content of `main.jsx` with the following.
...
- **`on:click={() => count(count() + 1)}`** - Updates the [signal](/reference/core/signal) value
...
- **Reactive styling** - The `bind:class` attribute changes automatically based on `isEven()`
