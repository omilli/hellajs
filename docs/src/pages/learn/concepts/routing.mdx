...
Protect routes using `before` hooks. Guards execute **before handlers** and can redirect users, but **do not block handler execution** — the handler always runs regardless of what the `before` hook does. For true access control, check auth state inside the handler or at the component level.
...
const requireAuth = () => {
  // Redirect happens, but handler still executes after
    ...
    '/admin': {
      ...
      children: {
        '/users': {
          before: requireAdmin,
          handler: (params, query) => {
            // ⚠️ This still runs even if requireAdmin redirected
            // Guard the view state inside the handler for safety
...
```typescript
type RouteInfo = {
  handler: Handler | null;        // Current route handler function
  params: Record<string, string>; // URL parameters (:id, etc.)
  query: Record<string, string>;  // Query string parameters
  path: string;                   // Current pathname + search
  meta?: Record<string, unknown>; // Route metadata (if defined on route)
};
```
...
const CurrentPage = () => (
  <div class="page-info">
    ...
    {route().params.id && <p>Item ID: {route().params.id}</p>}
  </div>
);
...
The [navigate](/reference/router/navigate) function provides programmatic routing with support for parameters, query strings, scroll control, and metadata. Navigation is asynchronous — route state updates on the next microtask, not immediately after the call.
...
const TodoList = () => {
  ...
  return (
    <div>
      ...
      <button on:click={() => searchTodos('urgent')}>
        Search
      </button>
      <button on:click={() => navigate('/todos/new')}>
        Add Todo
      </button>
    </div>
  );
...
Call `navigate` with the path and an options object containing params, query, and other settings.
...
// Navigate with scroll override
navigate('/long-page', { scroll: 'top' });

// Navigate with metadata
navigate('/dashboard', { meta: { requiresAuth: true } });
...
router({
  ...
  // mode: "history" is the default
});
```

### Hash Mode

Uses URL hash fragments for client-side routing, ideal for static file hosting without server configuration.

```jsx
import { router, navigate } from '@hellajs/router';

// URLs become: #/, #/about, #/users/123
router({
  mode: 'hash',
  routes: {
    '/': () => showHome(),
...
navigate('/users/123'); // Automatically becomes #/users/123

// With options
navigate('/search', { query: { q: 'test' } }); // Becomes #/search?q=test
```

## Route Metadata

Attach arbitrary metadata to routes for analytics, SEO, or feature flags.
...
// Access metadata reactively
const PageTitle = () => {
  const meta = route().meta;
  return <title>{meta?.title || 'App'}</title>;
};
```

## Scroll Behavior

Control scroll position after navigation with built-in options or custom logic.
...
router({
  scrollBehavior: (to, from) => {
    // Return scroll position or null to skip
    if (to.startsWith('/docs')) {
      return { top: 100, left: 0 }; // Scroll docs pages
    }
    return null; // Skip scrolling for other routes
  },
  routes: { /* ... */ }
});
```

### Route-Level Scroll Override

Individual routes can override global scroll behavior or disable it entirely.

```jsx
router({
  scrollBehavior: 'top', // Global default
  routes: {
    '/': () => showHome(),
    '/feed': {
    ...
    '/chat': {
      scroll: false, // Explicitly disable scroll handling
      handler: () => showChat()
    }
  }
});
...
### Inline Scroll Override

Individual navigate calls can override all scroll settings with the `scroll` option.

```jsx
// Override global/route scroll for this navigation only
navigate('/long-page', { scroll: 'top' });
...
**Scroll Priority:** Inline `navigate()` scroll > Route-level `scroll` > Global `scrollBehavior`
