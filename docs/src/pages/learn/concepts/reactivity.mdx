...
Both [`signal`](/reference/core/signal) and [`computed`](/reference/core/computed) use reference equality comparison to detect changes in objects and arrays.
...
// ✅ Setting new arrays triggers updates (reference equality)
todos([
  { id: 1, text: 'Learn HellaJS', done: false },
  { id: 2, text: 'Build an app', done: true }  // Changed done to true
]);  // Triggers update - new array reference

// ⚠️ Setting new arrays always triggers updates, even with identical content
todos([
  { id: 1, text: 'Learn HellaJS', done: false },
  { id: 2, text: 'Build an app', done: true }  // Same content as before
]);  // Still triggers update - different array reference
```

### Performance Considerations

Reference equality comparison is fast but means any new object or array instance triggers updates, even with identical content.
...
// ❌ Unnecessary updates - creating new array instances repeatedly
const items = signal([1, 2, 3]);
setInterval(() => {
  items([1, 2, 3]); // New array triggers update even though content is same
}, 1000);

// ✅ Check before updating to avoid unnecessary updates
const items = signal([1, 2, 3]);
const updateItems = (newItems) => {
  const current = items();
  // Only update if actually different (custom equality check)
  if (JSON.stringify(current) !== JSON.stringify(newItems)) {
    items(newItems);
  }
};

// ✅ Break large objects into smaller signals for fine-grained reactivity
const userProfile = signal({ name: 'John', email: 'john@example.com' });
const userPreferences = signal({ theme: 'dark', notifications: true });
// Now changing preferences won't trigger profile updates
```
...
An [`effect`](/reference/core/effect) function can nest other effects. Each nested effect creates a dependency on the parent.
...
effect(() => {
  ...
  // Inner effects also track activeUserId through parent
  effect(() => {
...
// When activeUserId changes, all effects re-run
activeUserId(2);
```

For proper cleanup of side effects (like intervals or event listeners), use the cleanup function returned by [`effect`](/reference/core/effect) or use [`scope`](/reference/core/scope) to manage multiple effects together.
...
Errors in reactive functions can break dependency tracking. Signals must be read BEFORE any errors occur to establish dependencies.
...
// ❌ Error thrown before reading dependencies breaks tracking
...
// ✅ Read all dependencies before potentially throwing
const safeComputed = computed(() => {
  const shouldThrow = riskyOperation();
...
// ✅ Or handle errors and return fallback values
const safestComputed = computed(() => {
  try {
    if (riskyOperation()) {
      throw new Error('Computation failed');
    }
...
// ⚠️ Effects: Read signals BEFORE try-catch to maintain tracking
effect(() => {
  const shouldThrow = riskyOperation();
  const value = data();
