...
Effects can create other effects, which are automatically disposed when the parent effect re-runs.
...
For side effects that require manual cleanup (like intervals or event listeners), store cleanup references externally or use [scope](/reference/core/scope) to manage effect lifecycles.
...
Group [signal](/reference/core/signal) updates into a single execution using [batch](/reference/core/batch). Effects are deferred and run once after all signal updates complete.
