...
config.apiUrl('new-url'); // ❌ Returns current value, argument ignored (readonly getter)
...
Stores expose [snapshot](/reference/store/store#snapshot), a plain object representation of your store data.
...
console.log(inventorySnapshot.items); // Plain array, not a live reference
...
The `update` method accepts either a partial object or a function for ergonomic mutations.
...
// Mutable update with function - mutate draft directly
user.update(draft => {
  draft.name = 'Jane';
...
Pass a function to `update` for ergonomic array operations.
...
Validate and transform values before they're stored.
...
- **Effects** run *after* changes - logging, persistence, derived state
- **Middleware** runs *before* changes - validation, transformation
...
// Note: store.update() batches its internal signal updates automatically
```
