...
Stores expose [snapshot](http://localhost:4321/reference/store/store#snapshot), a plain object representation of your store data.
...
The `update` method accepts either a partial object or a function for ergonomic mutations. The `set` method replaces the entire state.
...
user.set({
  ...
  age: 25,
  address: { street: '789 Oak St', city: 'Newtown', zip: '54321' }
});
```
