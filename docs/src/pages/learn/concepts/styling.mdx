...
Every CSS object receives a unique class name via an internal counter (`c1`, `c2`, etc.), ensuring isolation without additional configuration. Identical CSS objects share the same class name through memoization.
...
CSS Object → Hash Key → Cache Lookup → Class Name
    ↓           ↓            ↓             ↓
{color:'red'} → key → Hit: return c1 / Miss: create c2
```
