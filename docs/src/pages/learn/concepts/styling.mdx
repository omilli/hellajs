...
The CSS system processes JavaScript objects into CSS rules through deterministic key generation, providing predictable performance and automatic scoping.

**Processing Features:**
- **Rule Generation** - CSS rules generated synchronously from JavaScript objects
- **Memoization** - Identical CSS objects return the same cached class name
...
```
CSS Object → Hash Key → Cache Lookup → Class Name
    ↓           ↓            ↓             ↓
{color:'red'} → key → Hit: return c1 / Miss: create c1
```

Caching layers.
- **Deterministic Hash Keys** - `stringify(obj) + options` produces a unique key for each CSS object
- **Inline Cache** - Maps hash keys to generated class names for instant lookup
- **Reference Counting** - Tracks usage for automatic cleanup via `cssRemove()`

Performance benefits.
- **Reduced CSS Size** - Duplicate styles create only one CSS rule
...
Every CSS-in-JS style receives automatic scoping through a sequential class name counter, ensuring complete style isolation without configuration.

```
Input:  css({ padding: '1rem', color: 'blue' })
Output: '.c1{padding:1rem;color:blue}'
```

Scoping mechanisms.
- **Automatic Scoping** - Sequential counter generates unique class names (`c1`, `c2`, ...)
- **Manual Scoping** - Optional prefixes for component libraries
- **Global Scoping** - Bypass scoping for application-wide styles
- **Memoization** - Identical CSS objects share the same class name