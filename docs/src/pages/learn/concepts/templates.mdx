...
2. **`onMount`** - after element is in DOM (async via requestAnimationFrame)
3. **`onBeforeUpdate`** + **`onUpdate`** - on reactive updates (not during initial render)
4. **`onBeforeDestroy`** - before removal
5. **`onDestroy`** - after removal and effect cleanup

### When to Use Each Hook

- **`effects`** - For reactive logic that should cleanup automatically (logging, analytics, derived state)
- **`onBeforeMount`** - For initialization logic before DOM creation
- **`onMount`** - For DOM measurements, third-party library initialization
- **`onBeforeUpdate`** / **`onUpdate`** - For responding to reactive changes
- **`onBeforeDestroy`** - For canceling pending operations, saving state
- **`onDestroy`** - For cleanup of external resources (timers, WebSockets, subscriptions)
...
3. When clicked, the global listener routes the event to your handler via property lookup
