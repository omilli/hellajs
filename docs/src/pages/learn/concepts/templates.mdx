...
The [element](reference/dom/element) function provides DOM manipulation with reactive support, allowing you to imperatively interact with existing DOM elements. Operations execute immediately for connected elements or queue for disconnected elements.
...
// Execute callback when element loads
element('.widget').onLoad(() => console.log('Widget ready'));
