...
Control data freshness with `staleTime` - when data becomes "stale" but is still usable, `.get()` returns it immediately while fetching fresh data in the background.
...
// Timeline:
// 0-30s: Fresh - get() returns cached data instantly
// 30s-5min: Stale - get() returns cached data + background fetch
