...
Resources can be created directly from URLs for simple GET requests. The URL is used as both the fetch target and the cache key.

```jsx
// Simple URL-based resource
const users = resource('/api/users'); // URL is the fetch target AND cache key
...
Set `refetchOnKeyChange: true` to automatically refetch when key dependencies change, eliminating manual `invalidate()` calls.

```jsx
// Resource with key-change refetch enabled
const userId = signal(1);
...
console.log(posts.isLoading()); // true during key-change refetch
...
// With refetchOnKeyChange: true, no need to call user.fetch({ force: true })
...
// Switch users - automatically refetches with refetchOnKeyChange: true
userId(2); // No need to call invalidate() with refetchOnKeyChange mode
```
...
// 5min+: Expired - fetch() makes fresh network request
```
...
Control when resources should fetch data using the `enabled` option. Note that `enabled` is evaluated once at creation time — it is not reactive.

```jsx
// Conditional resource with enabled option
const userId = signal(null);
const isAuthenticated = true; // Static condition evaluated at creation time

const userData = resource(
  id => fetch(`/api/users/${id}`).then(r => r.json()),
  {
    key: () => userId(),
    enabled: isAuthenticated && !!userId() // Evaluated once at creation
  }
);

// If enabled is false, fetch() is a no-op
userData.fetch({ force: true }); // Won't fetch if enabled evaluated to false

// For reactive conditional fetching, use refetchOnKeyChange with a conditional key:
const userData2 = resource(
  id => fetch(`/api/users/${id}`).then(r => r.json()),
  {
    key: () => isAuthenticated ? userId() : undefined,
    refetchOnKeyChange: true
  }
);

// Or manually control fetching:
const logoutUser = () => {
  userId(null);
...
// slowResource.status() -> 'idle' (abort does not set error state)
// slowResource.data() -> { data: 'Initial' } (initialData preserved)
```
...
console.log(abortableResource.error()); // undefined (abort does not set error state)
console.log(abortableResource.status()); // 'idle'
```
...
// With refetchOnKeyChange: true, polling starts automatically
...
Resources can automatically refetch data when the user returns to the tab. This is useful for keeping session data or notifications fresh. The refetch only triggers when the tab becomes **visible** — hiding the tab does not trigger a fetch.
...
<div role="alert" class="alert alert-info alert-soft text-base">
  <Icon name="ph:lightbulb" width={24} height={24} />
  <span><strong>With <code>refetchOnKeyChange: true</code>:</strong> the initial fetch and focus listener start automatically. Without it, you must manually call <code>fetch()</code> first — the focus listener will still work for subsequent refetches.</span>
</div>
...
Resources can automatically refetch data when the browser reconnects after going offline. This ensures users see fresh data after network interruptions. Like `refetchOnWindowFocus`, this works independently of `refetchOnKeyChange` — but pairing with `refetchOnKeyChange: true` ensures the initial fetch happens automatically.
...
const smartRetry = resource(
  () => fetch('/api/data').then(r => r.json()),
  {
    ...
    retryDelay: (attempt, error) => {
      ...
      // Shorter delays for unknown/transient errors
...
userEmail.fetch(); // Joins the same request
