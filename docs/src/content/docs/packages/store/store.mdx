...
You can prevent properties from being changed after the store is created using the `readonly` option. There are two ways to make properties readonly:
...
const book = store({
  ...
  readonly: ["title", "author"] // Only these properties will be readonly
});

book.publicationYear(2022); // ✓ Allowed
book.title("New Title"); // ✗ Error: Cannot write to readonly signal
```

### Make the entire store readonly
...
const constants = store({
  ...
  readonly: true // All properties become readonly
});

constants.API_KEY("new-key"); // ✗ Error: Cannot write to readonly signal
constants.settings.maxRetries(5); // ✗ Error: Cannot write to readonly signal
```

Readonly properties are automatically wrapped in `computed()` to prevent external writes while maintaining reactivity. This is fully type-safe - TypeScript will prevent attempts to write to readonly properties at compile time.

Note that readonly only applies to direct property updates. The store's `update()` method and calling the store as a function will still work to update the state, even for readonly properties. This allows controlled state updates while preventing direct mutations.