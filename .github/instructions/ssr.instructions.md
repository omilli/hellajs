...
| `lib/internal/resolve.ts` | `resolveValue`/`resolveAsync` (call-if-function, await-if-Promise resolvers); `isPromise` type guard (`@internal` — feeds the sync walker's thenable warn). |
...
- **Sync `ssr` warns on thenables.** A reactive child or function-ref prop that resolves to a Promise cannot be awaited by the sync walk — each occurrence warns (`[ssr] Promise value under sync ssr - use ssr.async or ssr.stream, got [object Promise] emitted`) and still emits the stringified value as before. `ssr.async`/`ssr.stream` await it (no warn).
