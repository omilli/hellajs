...
HellaJS's dedup is structurally distinct. The `OngoingRequest` shape stores `{ promise, abortController }` (`lib/internal/dedupe.ts`) in a `WeakMap<object, Map<unknown, OngoingRequest>>` keyed by fetcher reference, then cache key — structurally compared for plain objects/arrays via the same `stableKey` normalization as the cache (`lib/internal/key.ts`). When a second resource issues the same fetcher + key:
...
| Offline pausing | Opt-in `pauseWhenOffline` — reads defer and resume on reconnect with the stashed force flag; mutations not paused (`lib/resource.ts`) | Default-on `networkMode: 'online'` — `fetchStatus: 'paused'`, resumes on reconnect | Polling-only (`refreshWhenOffline`) | None | None | None |
