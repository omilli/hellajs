...
The `staleTime` option controls how long cached data is considered fresh. When data becomes stale, calling `.get()` returns the stale data immediately while triggering a background refresh.
