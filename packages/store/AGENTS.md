...
| `lib/internal/create.ts` | `createStore` factory: `$snapshot` computed over a keys signal, `$update` (incl. unknown-key materialization via the shared `materializeKey` closure), `$cleanup`, `$subscribe`, recursive init |
...
| `lib/types.d.ts` | `Store<T,R>` (its `update` member carries the widening `<P>` overload + `Simplify`, both shaping returned refs), `SettableKeyOf<T>` (non-exported, `$subscribe`-only) + `Simplify` (non-exported), `PartialDeep`, `Snapshot`, `StoreMiddleware`, `StoreEquals`, `StoreOptions`, `ReadonlyKeys`, `StoreAdaptor`, `PersistOptions`, `PersistHandle` |
