...
| `lib/types.d.ts` | `Store<T,R>` (its `update` member carries the widening `<P>` overload + `Simplify`, both shaping returned refs), `SettableKeyOf<T>` + `Simplify` (non-exported), `PartialDeep`, `Snapshot`, `StoreMiddleware`, `StoreEquals`, `StoreOptions`, `ReadonlyKeys`, `StoreAdaptor`, `PersistOptions`, `PersistHandle` |
