...
A store is built by `createStore()`, exposed through five `store()` overloads (`lib/store.ts`), walking `Object.entries(initial)` exactly once and emitting a per-property reactive primitive (`lib/internal/create.ts`). The output is a plain object whose properties are *real signal functions* attached via `Object.defineProperty`: non-writable for signal-backed leaves, nested stores, and the built-in methods, writable for preserved functions (`lib/internal/utils.ts`); there is no Proxy on the hot path and nothing intercepts reads after construction. Branching is type-based: function values are preserved as-is, plain objects recurse into nested stores, Array/Map/Set values install granular collection containers whose plain-object elements convert to element stores (`lib/internal/deep.ts` `installCollection`), and everything else (primitives, `Date`, `RegExp`, class instances) becomes a single `signal(value)` leaf (`lib/internal/create.ts`).
...
HellaJS makes each primitive property its own signal (`lib/internal/create.ts`). Two effects that read different properties of the same store never interfere; only the signal that actually changed propagates through core's graph. Array/Map/Set values are granular collection containers: one child signal per element plus a structural version (`signalArray`/`signalMap`/`signalSet` via `lib/internal/deep.ts` `installCollection`), with plain-object elements converted to element stores at construction, so a field write wakes exactly that field's readers and a plain-object write over an element store patches in place (`lib/internal/deep.ts` `merge`, verified by `tests/collections.test.ts`). The honest line against the proxy frameworks: per-element value granularity, in-place collection ops, and field-precise patches are here without any Proxy, but depth is construction-time conversion, not deep element proxies, and structural granularity is container-level (any structural op wakes every positional reader), weaker than Vue's per-key tracking. Direct writes are reference-compared by default, but the `equals` option opts individual keys into content equality (`'structural'` reuses the draft comparator, or a custom `(previous, next)` comparator runs per element inside each child signal, after middleware), so an equal-content write wakes no subscriber (`lib/internal/create.ts`, verified by `tests/equals.test.ts`).
...
|---|---|---|---|
| HellaJS | Per-property signals; Array/Map/Set keys as granular containers with element stores | Yes, recursive (construction-time conversion) | Readonly via throwing guard; propagates deep |
...
- Array/Map/Set values map through `Reactive<T[K]>`: `Todo[]` becomes `SignalArray<Store<Todo>, PartialDeep<Todo>>` (reads give element stores, writes take raw elements or partial patches), `Map<string, Todo>` becomes `SignalMap<string, Store<Todo>, PartialDeep<Todo>>`, and readonly collection keys collapse to `() => T[K]` when the key is in `R` (`lib/types.d.ts`).
...
Its gaps are the predictable ones: ecosystem size (no devtools bridge, no Redux DevTools story), no deep element proxies (collection depth is construction-time element-store conversion: interface-typed elements fall to raw typing, `Set` object members match by element-store identity rather than raw identity), additions typed only through returned references (the original reference and draft mutators keep their narrower types; middleware and write equality are creation-keyed, while Valtio and MobX observe new properties on assignment without any declaration), no async or suspense primitives in the package itself (delegated to `@hellajs/resource`), and a whole-tree re-computation cost on `$snapshot()` that makes wide stores read better through individual signals. For applications living in the HellaJS ecosystem that want deeply reactive state with the strongest readonly typing at the smallest dependency cost, it is the leanest option here. For applications that need devtools, time travel, async flows, or assignment-driven growth, Valtio and MobX remain the safer bets.
