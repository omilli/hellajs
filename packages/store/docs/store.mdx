...
interface Store<T> {
  ...
  update(partial: PartialDeep<T> | ((draft: T) => void)): void; // Partial or mutable update
  cleanup(): void; // Dispose nested stores and computed values
}
...
// Nested stores are disposed; primitive signals remain functional
appStore.count(10);
```
