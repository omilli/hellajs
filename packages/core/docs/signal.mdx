...
```
- `initialValue`: The initial value for the signal. When omitted, the signal value is `undefined`.
...
A deep equality check prevents propagation to dependents when the value remains unchanged, avoiding unnecessary [effect](/reference/core/effect) executions.
