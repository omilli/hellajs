...
**Verdict:** HellaJS and Solid are the only two routers here that intercept plain `<a>` clicks by default, and HellaJS's sync `before` hooks can cancel or redirect. The navigation gaps are link prefetching (Next.js-only at hover granularity) and async-blocking guards (Vue/Angular/TanStack accept `Promise<boolean>`; HellaJS treats a `Promise` return as "pass").
...
**Verdict:** HellaJS ships real sync guards — cancel, redirect, and (on popstate/hashchange) URL restoration via `replaceState` (`lib/router.ts`) — matching Vue/Solid/Angular/TanStack for synchronous auth-style gates. The asymmetry is async: a `Promise`-returning `before` cannot block (the pipeline is sync), so flows that need to await a server check before committing a navigation require handler-side revalidation or a different router.
