...
| `href.ts` | The `href` export: validation + `internal/path.ts buildPath`, returns the built URL string — no history, no resolution. |
  ...
  | `hooks.ts` | `executeHook` (arity dispatch + try/catch + promise.catch), `executeNotFound` (try/catch, logs `[router] notFound:`), `executeGlobalHook` (`(to, from)` paths). |
  | `utils.ts` | `EMPTY_OBJECT`/`EMPTY_CRUMBS`, `hasChildren`, `getHashPath`, `stripBase`, `sortRoutesBySpecificity`, `getCachedRouteEntries` (identity-keyed `entryCache` WeakMap of per-map entry lists — `nested`/`child`/`raw` variants, read-through on first sight). Leaf module — no internal imports. |
