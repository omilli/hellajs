...
| `href` | `href.ts` | Typed URL builder: same `:param`/`:param?`/`*`/query semantics as `navigate` via shared `buildPath`; returns the string, no history touched. |
  ...
  | `type *` | `types.d.ts` | `RouterConfig`, `RouteInfo`, `RouteWithHooks`, `Handler` (full `(params: Params, query: Params) => unknown` signature; runtime dispatch is arity-based), `NavigateOptions`, `Redirect`, `ScrollBehavior`, `HistoryMode`, `Crumb`, `ExtractParams`, `Params`, `Routes`, `RouteValue`, `GlobalHooks`. |

  ## File map

  | File | Responsibility |
  |---|---|
  | `router.ts` | Config → state signals (validates + normalizes `base`, throwing on non-`'/'`-prefixed values); initial-path detection (`url` override / hash vs history / memory → `/`; base stripped from every read); popstate/hashchange + click listeners attached via the local `attachListeners` helper with composed cleanup (none attached in memory mode; pop handlers pass `isPop` so pops pop the scroll stack); synchronous `updateRoute()` on return. |
  | `internal/state.ts` | 9 config signals (`routes`, `hooks`, `redirects`, `notFound`, `mode`, `base`, `scrollBehavior`, `previousPath`, `inheritMeta`). |
...
| `href.ts` | The `href` export: validation + `internal/path.ts buildPath`, returns the built URL string — no history, no resolution. |
  | `internal/match.ts` | `parseQuery`, `matchPattern` (segment/optional/wildcard extraction), `matchSegments` (recursive aligner, `:name?` consume-first backtracking), `matchNestedEntry` (single-entry chain resolver), `matchNestedRoute` (specificity-sorted loop over entries, params spread-merged), `matchRoute` (flat). |
  | `internal/hooks.ts` | `executeHook` (arity dispatch + try/catch + promise.catch), `executeNotFound` (try/catch, logs `[router] notFound:`), `executeGlobalHook` (`(to, from)` paths). |
  | `internal/utils.ts` | `EMPTY_OBJECT`/`EMPTY_CRUMBS`, `hasChildren`, `getHashPath`, `stripBase`, `sortRoutesBySpecificity`, `getCachedRouteEntries` (identity-keyed `entryCache` WeakMap of per-map entry lists — `nested`/`child`/`raw` variants, read-through on first sight). Imports only `internal/core.ts`; the leaf below it. |
  | `internal/resolve.ts` | Resolution pipeline: `RouteVerdict` (`matched`/`cancelled`/`redirected`/`deferred`) + sync hop counter (`updateRoute`) + async `navEpoch`/`asyncHops` (deferral supersede + cross-await loop cap), `tryRedirect`, `tryMatchRoute` → `matchNestedPhase`/`matchFlatPhase` via shared `commitMatch` + `mergeRouteMeta`, `buildRouteInfo` (writes `pending: false`), `go` (builds the `historyCommit` closure — committed inline on a sync `"matched"`, captured by the deferred continuation otherwise; `base`-prefixed in history mode; memory mode commits none). Pop-aware scroll stack: `go` captures `window.scrollX/scrollY` before resolution and pushes onto `scrollStack` (`internal/scroll.ts`) on non-replace commits; pop commits (`isPop` threaded through `updateRoute` → `commitMatch` → `handleScroll`) pop it via `takeSavedScroll`; `resetAsyncNavigation` backs `resetRouter`. |
  | `internal/matched.ts` | `extractHandler`/`Meta`/`extractInheritMeta`/`Scroll`/`extractRouteHooks` (before/after/leave), the guard-verdict machine (`GuardVerdict` incl. `{ deferred }` + `interpretGuardResult` — the single client/server fork: a `Promise` return defers on the client, proceeds + catch-logs with no window), the leave-guard chain snapshot (`setMatchedChain` — root→leaf values of the last committed match, cleared on notFound commits and `resetRouter`) + `runLeaveGuards` (global `hooks.leave(to, from)` first, then chain `leave` child→parent with the departed route's params/query), `runGuardsNested`/`runGuardsFlat` (leave phase, then shared global-before prologue; the global hook receives `(toPath, route().path)`), `executeRouteWithHooks` (threads `(to, from)` into `global.after`). |
| `internal/scroll.ts` | The scroll state machine: `scrollStack` (saved positions, one per push commit — replaces and init never push), `takeSavedScroll` (pops on browser back/forward), `resetScrollStack`, `handleScroll` (priority inline `navigate({scroll})` > route-level `scroll` > global `scrollBehavior`; custom fns receive `(to, from, savedPosition)` — `savedPosition` passed only on pops; custom-fn throws caught and logged `[router] scrollBehavior:`). |
| `internal/path.ts` | `buildPath` — the single substitution truth (`:param` → `encodeURIComponent`, absent `:param?` strips token + preceding slash, `*` → raw insert, strip unmatched, query serialize); shared by `navigate` + `href`. |
  | `internal/core.ts` | Re-exports `signal`, `isFunction`, `isString`, `isPlainObject`, `isObject`, `isNull`, `hasWindow` from `@hellajs/core`. |
  ...
  <non-obvious>
    ...
    **Scroll no-op on init** — `previousPath` is seeded with `initialPath` (`router.ts` previousPath seed), so the first `updateRoute()` sees `from === to` and skips (`internal/scroll.ts handleScroll`). **Scroll priority**: inline `navigate({scroll})` > route-level `scroll` > global `scrollBehavior`; `false` at any level disables; `"auto"`/`"preserve"` skip `scrollTo`; custom fn returning `null` skips. **`savedPosition`**: `go` captures `scrollX/scrollY` before resolution and pushes it on non-replace commits (stack mirrors pushState entries — init and replaces never push); popstate/hashchange handlers pass `isPop`, and the commit pops the stack into the custom fn's third arg (null on empty stack). String presets ignore it — `"top"` scrolls top even on back. Forward-after-back loses the forward entries (no `history.state` ownership — documented imprecision).
  ...
  - **Identity-keyed route-entry cache** — each route map's entry lists (`nested`/`child` sorted, `raw` insertion order) are built once and reused across navigations at every nesting level (`utils.ts getCachedRouteEntries` over the `entryCache` WeakMap) — no per-navigation `filter().sort()` allocation. Invalidation is map identity: `router()` with a fresh object misses the cache, abandoned maps GC; in-place mutation of a registered map is invisible (reconfigure with a new object).
  - **Single `route` signal** — all routing state co-located; no per-route subscriptions.

  ## Testing

  - Run `bun coverage router` (never bare `bun test` — stale bundles; `guides/tests.md` §Triage & Gate Semantics). Shared helpers in `tests/helpers.ts`: `setupRouterEnv` (resetTestState + setupContainer + real-URL seed via `window.location.href`, optional `pageUrl` defaulting to `"http://localhost/"`), `expectLoggedError` (asserts `[router]` prefix against `suppressConsole` output). One behavior per file: `routing`, `hooks`, `hook-arity`, `redirects`, `specificity`, `history`, `hash-mode`, `navigate-options`, `href`, `intercept`, `errors`, `guards`, `async-guards`, `leave-guards`, `inherit-meta`, `meta`, `crumbs`, `active`, `scroll`, `url-encoding`, `atomicity`, `base-path`, `memory-mode`, `reset-router`, `ssr`, `validation` — conventions per `guides/tests.md`; `flush()` is sync.
