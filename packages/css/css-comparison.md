...
| Shadow DOM / custom insertion point | Yes — per-call `host` on `css()`/`cssVars()` (`lib/internal/sheet.ts`) | Via cache `insertionPoint` | Via `StyleSheetManager target` | No | No |
