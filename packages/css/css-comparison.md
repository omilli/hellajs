...
- **Symmetrical reference counting with DOM cleanup at zero** — `removeCss()` and `removeCssVars()` decrement per-call ref counts and remove the actual CSSOM rules / scope rules only at zero (`lib/css.ts`, `lib/vars.ts`).
