...
- At-rules split by category (`CONDITIONAL_AT_RULES` in `lib/css.ts`): conditional rules (`@media`, `@container`, `@supports`, `@starting-style`) inherit the active selector (under a scope, their bodies re-scope to that class), while a selector-less conditional body containing direct declarations throws instead of emitting a block the browser would drop, and so do top-level declarations with no selector or at-rule at all (same loud-rejection family, `lib/css.ts`); definitional rules (`@keyframes`, `@font-face`, `@layer`, `@import`, …) always process with an empty selector so they never nest under a class, and an `@`-prefixed scalar key is a block-less statement (`@import url(…);`) hoisted ahead of all braced text in the emission and inserted ahead of the first braced rule client-side (`lib/css.ts`, `lib/internal/sheet.ts`).
- The produced CSS text splits into top-level rules at brace-depth boundaries via a quote-aware walk (braces inside quoted CSS strings such as `content: '}'` never count toward depth, so an exotic value can neither truncate its own rule nor swallow the rules after it), and each rule is inserted individually through `upsertRule`: `CSSStyleSheet.insertRule`/`deleteRule` against `el.sheet`, with an `indexMap` keyed `id:key` that early-returns when the existing rule's `cssText` is unchanged (`lib/internal/injection.ts`, `lib/internal/sheet.ts`). The CSSOM is the only write path: there is no `textContent` mirror and no full-sheet rewrite on update (`lib/internal/sheet.ts`).
...
| `@media` / `@keyframes` / `@font-face` / `@container` / `@supports` / `@layer` / `@import` | Yes, all: conditional-vs-definitional scoping rules in `css()` plus a hashed `keyframes()` primitive, with `@import` as a block-less statement placed ahead of braced rules (`lib/css.ts`, `lib/keyframes.ts`, `lib/internal/sheet.ts`), and `layer` options on `style()`/`vars()` wrapping emitted rules in `@layer` (`lib/style.ts`, `lib/internal/vars.ts`) | Yes | Yes | Yes (`keyframes`, `fontFace`, `layer`, `@container`) | Yes (`@layer`-native) |
