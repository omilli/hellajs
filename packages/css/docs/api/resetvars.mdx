...
```jsx
import { vars, resetVars } from '@hellajs/css';
...
The `resetVars` function provides complete cleanup of the CSS variables system:
...
- **Reactive Effects**: Disposes all active reactive effects created by `vars`
...
const resetAllCSS = () => {
  ...
  resetVars();  // Clear CSS variables and effects
};
```
