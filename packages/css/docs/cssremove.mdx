...
Styles are injected once and deduplicated via an inline cache. All calls to `css()` with the same object return the same class name without re-injecting. One call to `cssRemove()` removes the rule.
...
// Multiple components reference the same style — CSS is injected once
const classA = css(sharedStyle);
const classB = css(sharedStyle); // same class name, cached
const classC = css(sharedStyle); // same class name, cached

// Remove the style — CSS is removed from DOM
cssRemove(sharedStyle);

// Calling css() again re-injects the rule
const classD = css(sharedStyle);
```
