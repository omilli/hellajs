...
describe("error boundaries", () => {
  ...
  test("catches error in bind callback", () => {
    ...
    mount(html`
      <div
        ...
        hook:onError=${(error: Error) => {
          errorThrown = error;
          return html`<span>Error in bind</span>`;
        }}
        bind:data-value=${() => {
  ...
  test("catches error in event handler", () => {
    ...
    mount(html`
      <div
        ...
        hook:onError=${(error: Error) => {
          errorThrown = error;
          return html`<span>Event error</span>`;
        }}
      >
        <button id="btn" on:click=${() => { throw new Error("Click error"); }}>Click</button>
      </div>
    `);
  ...
  test("error propagates when handler returns void", () => {
    ...
    mount(html`
      <div
        ...
        hook:onError=${(error: Error) => {
          parentError = error;
          return html`<span>Parent fallback</span>`;
        }}
      >
        <div
          id="child-boundary"
  ...
  test("reset function restores original content", () => {
    ...
    mount(html`
      <div
        id="boundary"
        hook:onError=${(_error: Error, reset: () => void) => {
          resetFn = reset;
          return html`<span>Error occurred</span>`;
        }}
      >
        ${() => {
          if (shouldThrow()) throw new Error("Recoverable");
          return html`<span>Original</span>`;
        }}
      </div>
    `);
  ...
  test("catches error in lifecycle hook", () => {
    ...
    mount(html`
      <div
        ...
        hook:onError=${(error: Error) => {
          errorThrown = error;
          return html`<span>Hook error</span>`;
        }}
        hook:beforeMount=${() => { throw new Error("beforeMount error"); }}
      ></div>
    `);
  ...
  test("works with html template hook: prefix", () => {
    ...
    const errorHandler = (error: Error) => {
      errorThrown = error;
      return html`<span>Template error</span>`;
    };
