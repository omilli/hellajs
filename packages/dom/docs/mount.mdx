...
HellaJS automatically manages reactive effects and event handlers through the [nodeRegistry](/reference/dom/noderegistry).
...
Reactive effects and DOM events are automatically cleaned up when elements are removed from the DOM via the [nodeRegistry](/reference/dom/noderegistry) system. No manual cleanup is required for standard reactive bindings.