...
| `portalCleanup` / `forEachCleanup` / `lazyCleanup` / `transitionCleanup` / `suspenseCleanup` | Optional component disposers. Every dynamic component registers on its **anchor** (its own persistent text anchor, or the hydrate region anchor under `adoptRegion`), so cleanup fires when the anchor is removed or drained (`drainAnchorCleanup`). |
...
- **Placeholders.** Interpolations become `__SLOT_N__` markers in the string and `{ placeholder: N }` markers in the AST. **Format is `__SLOT_N__`, not `__HELLA_N__`.** `parseTextContent` splits text and mixed attribute values containing slots.
- **Static-subtree optimization.** `markIfStatic` tags any node whose `props`/`on`/`e`/`hooks`/`error`/`children` contain no `placeholder` and no `dynamicComponent` as `static = true`; field values that are arrays (mixed-attribute parts arrays) are scanned element-wise for a `placeholder` marker. `cloneWithValues` short-circuits on `Object.hasOwn(node, "static")` and returns the node as-is — **static subtrees are shared by reference across every invocation of the same literal.** This is why mutating a returned HellaNode is unsafe. `mountNode` further caches the first-built DOM subtree in a `staticDom: WeakMap<HellaNode, Element | DocumentFragment>` (`render.ts`); subsequent mounts of the same `static` node return `cache.cloneNode(true)`, replacing O(nodes) DOM construction with O(1) clone (`render.ts:mountNode`). Safe because `markIfStatic` guarantees `static` nodes carry no `on`/`e`/`hooks`/`error` — zero handler/hook state exists on cached elements (a `component()`-wrapped static root adds only `componentScope` state, re-wired fresh per wrapper/clone). The cache is cleared by `resetDom()` for test isolation.
...
- **Close semantics.** Void elements (`VOID_TAGS` beside `TOKEN_REGEX`, mirroring `plugins/babel/src/constants.mjs`) and self-closed tags append as leaves and never push to the stack, so following content is a sibling. A closing tag closes the nearest open ancestor with a matching tag, implicitly closing everything above it; a closer with no match is stray and dropped. Dynamic-component closers (`</__SLOT_N__>`) close the nearest open dynamic component: the open and close markers are distinct expressions, so their indices never agree. Unclosed elements auto-close at EOF: children were appended to their open parent at open time, so only the outermost unclosed node (`stack[0]`) is flushed as a root, never re-added. No throw. `plugins/babel/tests/parity.test.ts` runs a shared corpus through this parser and the compiled transform to keep the two paths identical.
- **Cloning rules.** `placeholder`/`static`/`dynamicComponent` short-circuit; arrays flat-clone; HellaNodes shallow-clone each bucket; children `.flat()`-ed; a `props` value that is a mixed-attribute parts array concatenates into one string (mirrors the babel plugin's `+` concat of the same template).

## `mount(node, target = "#app")` (`lib/mount.ts`)

The handle scaffolding (`flush`/`unmount` closures, `attached`/`cancelled` state, thenable dispatch, `beginMountPhase`/`endMountPhase` bracket) is shared with `hydrate` via `createMountHandle` (`lib/internal/handle.ts`); only `attachImpl` differs, and `hydrate`'s optional `afterFlush` arg starts the deferred-region watch after the first `flush()`. `attachImpl` returns the `mountedNodes` root array the handle's `unmount()` iterates (`cleanupSubtree` + `parentNode?.removeChild` each); mount's is `Array.from(container.childNodes)` after `replaceChildren`, so a spread fragment root's children all unmount.
...
## `hydrate(node, target = "#app")` (`lib/hydrate.ts`, `lib/internal/hydrate.ts` + `internal/deferred.ts`)

Attaches reactivity to existing server-rendered HTML in place — re-executes the component tree and wires effects/handlers/state to the DOM the server shipped, **never `replaceChildren`** (the core invariant vs `mount`). Mirrors `mount`'s resolve + sync/async shape; `attach` hydrates the resolved node against `container`'s existing childNodes (no replace). Fragment root → `hydrateSequence` over `container.firstChild` FIRST, then wire the root's `componentScope` onto the re-derived first surviving node (`wireFragmentScope`, leading `[`/`]` markers skipped — post-walk it is a leading inner anchor or the leading adopted static node; hydrate anchors lead their region, so this carrier survives every child swap); single element → `hydrateNode(node, container.firstChild)`. Empty container → falls back to a fresh `mount`. Returns the same `MountHandle` shape as `mount`; each path returns its `mountedNodes` roots to the handle (`Array.from(container.childNodes)` on the fresh-mount and fragment paths, `[container.firstChild]` on the single-element path), so `unmount()` removes every adopted fragment-root child.

- **`hydrateNode(node, existing, boundary?)`** — mirrors `mountNode`'s step order: `static` fast-path (verify tag, attach nothing — static subtrees carry no `on`/`e`/`hooks`; a `component()`-wrapped static root still wires `componentScope` onto the adopted element so cleanup disposes it); copy `componentScope`/`error`; register hooks; run `beforeMount`; **SKIP `props`** (server applied them via `ssr`); register `on:`/`e:` and wire function-ref props against `existing`; recurse `hydrateSequence`. Tag mismatch or missing element → `console.warn("[dom] hydrate mismatch…")` + `replaceMismatch` (`mountNode` subtree-replace via `existing.parentNode.replaceChild`, created in `existing`'s namespace via `childNamespaceOf`); a missing child (no existing) returns an orphan that `hydrateSequence` appends. Tag comparison is uppercase-folded for HTML elements but exact-case for foreign-namespace elements (SVG/MathML keep authored case — `clipPath` ≠ `CLIPPATH`), via the local `tagMatches` helper.
- **`hydrateSequence(parent, children, current, boundary)`** — a **marker-reader**: walks AST children in parallel with existing DOM via a node pointer, locating each dynamic region by its `<!--[->…<!--]-->` Comment markers (`isMarkOpen` / `gatherRegion` / `consumeRegion`). Static text/elements match by position (consume one node); element children → `hydrateNode` (adopt); a fragment child → gather + remove its marker pair, recurse inline, THEN wire its `componentScope` onto the re-derived first region node (`regionPrev.nextSibling` / `parent.firstChild`, leading `[`/`]` markers skipped — post-recursion it is a leading inner anchor or the leading adopted static node, both swap-stable; wiring pre-recursion would land on an inner open marker the walk itself removes; empty region → dispose before recursing); a reactive child → `consumeRegion` (gather nodes, remove markers, insert anchor) + `adoptReactiveRegion` (adopt the gathered nodes first-run, clear+render on subsequent runs — mirrors `appendToParent`, incl. the isDynamic-resolved `Proxy` branch which is safe here because `clearRenderedNodes` runs before re-rendering; each re-run first calls `drainAnchorCleanup(anchor)` so component state left on the persistent region anchor never outlives its run — a no-op for the common text/element case); an isDynamic child → `hydrateDynamic`.
- **`HydrateCtx` + stack** (`peekHydrateContext`/`push`/`pop`) — an **internal** type in `lib/internal/hydrate.ts` (NOT in `nodes.d.ts`; `RenderFn`'s public signature is unchanged). Carries `{ anchor, existingNodes, hydrateNode }`. `adoptRegion` pushes it around each isDynamic `fn(parent)` call so the component reuses the walker's pre-positioned anchor and adopts the marker-gathered region nodes instead of building fresh. Reentrancy-safe for nested regions. `adoptRegion` + `swapSuspenseStage` are exported `@internal` and registered into `internal/deferred.ts` via `setDeferredAdopters` at module init (the `setMountNode`-pattern cycle break: deferred.ts needs the walker-coupled fns, hydrate.ts needs the deferred-state accessors, so deferred.ts imports no hydrate.ts values).
- **isDynamic dispatch** (`hydrateDynamic`): `consumeRegion` gathers the region's nodes + positions the anchor; `adoptRegion` pushes the ctx + calls `fn`. `ForEach`/`Transition` adopt the gathered nodes; `Portal` passes `[]` (server rendered nothing in-place) and re-mounts into the target; `Lazy` adopts the gathered nodes too — server content (a loaded render from `ssr.async`/`ssr.stream`, or a sync render's loading UI) stays until the loader's fresh run swaps it (`Lazy.ts` skips the loading insert when the region is non-empty and `clearRenderedNodes`s the server nodes on settle — no flash). `Suspense` runs `swapSuspenseStage` — the no-script/HappyDOM fallback (a staged `<template>` whose id matches a sentinel comment replaces the fallback; the swap also returns the sentinel comment itself). In a browser the inline `$hs` swap script (emitted by `@hellajs/ssr`) has already swapped each region as it arrived, so this runs only when that script hasn't (e.g. in tests). Either way it then adopts the resolved children. When the sentinel is present but its staged `<template>` never arrived (interrupted stream), the ctx is flagged `stageMissing` and `<Suspense>` degrades to fresh-mount semantics — fallback kept, child re-suspended client-side, content swapped on settle, rejections bubbled — EXCEPT while `document.readyState === "loading"` (hydrate executing mid-parse), where the region is instead registered in `deferredRegions` (`deferSuspenseRegion`, `internal/deferred.ts`): the fallback stays mounted, `fn` is NOT called, and `hydrate`'s attach starts the deferred-region watch (`startDeferredRegionWatch`, `internal/deferred.ts`) — a MutationObserver on the hydrate container (`childList + subtree`) calling `recheckDeferredRegions` on mutation (template arrived → swap+adopt; sentinel detached + fallback replaced externally → adopt what now occupies the region slot, anchor → recorded `next`), a raw `document.body` capture listener buffering the discrete set `{click, mousedown, mouseup, keydown, change}` whose `composedPath()` crosses a pending region's recorded nodes, and a `document` `readystatechange` listener that drains remaining regions to `stageMissing` degrade when the document completes (dead stream). On adoption, buffered events replay POSITIONALLY — on the original target while connected, else on the adopted node at the same top-level index (a detached target cannot reach the delegated body listener, and the handler lives on the resolved content); entries with no connected counterpart are dropped; an `isReplaying` guard prevents re-buffering; synthesized `new Event(type, { bubbles, cancelable, composed })`, no coordinate fidelity. The watch tears down at zero pending; the inline `$hs` no-ops for a deferred region (hydrate consumed its markers — the balance walk finds no open `[`), leaving the template for the watcher. ForEach's first-render adopts via `hctx.existingNodes` into `keyToNode`/`keyToItem`/`currentKeys` **iff `existingNodes.length === arr.length`** (count-strict); on mismatch it warns + removes the gathered nodes + fresh-builds (the LIS update path is unchanged). `Transition` adopts `existingNodes[0]` as `current` when visible (applies `appear`).
- **Marker contract** — `ssr()` wraps every dynamic region (reactive child, isDynamic component, nested fragment) in `<!--[->…<!--]-->` Comment markers; hydrate reads them. There is **no coalescing/rebuild**: each reactive value is its own bounded region, adopted in place. Missing markers (e.g. hand-built server HTML without them) → `console.warn("[dom] hydrate: expected … marker, not found")` + fresh-mount the region (graceful degradation). Reset: `resetHydrateState()` clears the stack and delegates its deferred half to `resetDeferredState()` (`internal/deferred.ts` — registry + replay queue cleared, watch torn down; wired into `resetDom()`).
...
- **Fragment-root scope wiring.** The `tag: "$"` branch returns before the element wiring, so the scope rides the fragment's **last child** (`chainScopes` — the child can already carry an inner component's scope, e.g. `html\`<${B} /> tail\``); an empty fragment disposes immediately (no DOM node owns the scope). A text last child is legal: `collectRemovedNode` queues state-carrying text nodes on removal. Last, not first, because mount anchors TRAIL their content — every function-valued child (reactive getter, `ForEach`, `Lazy`, `Transition`, `Suspense`) appends its persistent anchor, then inserts rendered nodes `insertBefore(node, anchor)` — so the last node survives every child swap while a leading rendered node is disposable; a static tail persists until subtree removal. The mirror image holds hydrate-side, where anchors lead (see §hydrate).

- **Static fast-path scope wiring.** A fully-`static` root returned by `component()` arrives as a fresh per-invocation wrapper (`{ ...result }` — the shared template node is never mutated), and a `staticDom` cache-hit clone gets `node.componentScope` re-wired onto the clone's own state (`cloneNode` carries no `ElementState`; a fragment clone wires its last child — children move out on insert, the clone itself is dead state), so every mount of the same node object (incl. `reset()` re-mounts) disposes its scope on cleanup.
...
- **Reactive child effect** (non-dynamic function child): creates a text anchor + `renderedNodes[]` + one effect. Each run resolves the value, `cleanupSubtree` + `removeChild` on every previous node, re-inserts. If the resolved value is itself a dynamic function, a `Proxy` parent intercepts `appendChild` to track nodes while still inserting before the anchor. The resolved component registers its effect and disposer slots on its **own anchor** (tracked by the intercepted `appendChild`), so switching away removes its output and disposes its effect via `clean(anchor)` — and its internal runs do not fire the parent element's `beforeUpdate`/`afterUpdate` hooks. Errors `phase: 'mount'`, fallback `insertBefore`-ed at anchor — **preserves siblings**, unlike the prop-effect/event path.
...
- **Sync `cleanupSubtree(root)`** — called directly by `appendToParent` (reactive child swap), ForEach (stale-removal + list clear), Transition (leave completion). `traverseDescendants` (iterative stack) → per descendant `clean(node)`: `beforeDestroy` → `componentScope?.()` → `portalCleanup?.()` → `forEachCleanup?.()` → `lazyCleanup?.()` → `transitionCleanup?.()` → `suspenseCleanup?.()` → drain `effects` → `removeDirectHandlers` → `afterDestroy` → `deleteState`. `drainAnchorCleanup(node)` (beside `clean`) runs the same five component slots minus `componentScope` and drains `effects` **without** `deleteState` — the persistent hydrate region anchor survives re-renders, only its per-run occupants are disposed.
- **Scoped observer safety net.** `registerContainer(container)` (called from `mount()`; accepts an `Element` or a shadow root) + `ensureContainerObserver` lazily create one `MutationObserver` shared across all mount targets (`observedContainers: WeakSet<Element | ShadowRoot>`), observing `{ childList: true, subtree: true }`. Removed nodes' traversed via `collectRemovedNode` (exported `@internal` from `queue.ts`; the refObserver's removal walk reuses the same collector), which collects removed nodes with state → `cleanupQueue` — a stateful node is queued WITHOUT recursing into it (`processCleanupQueue`'s `cleanupSubtree` already traverses its descendants; only stateless intermediates are walked through); state-carrying **text** nodes queue too (a fragment root's scope can ride a text child — stateless text keeps the cheap skip); added element nodes → `mountQueue` (scheduled only when `mountHooksExist`); both drain on `queueMicrotask`. `processCleanupQueue` skips nodes still `isConnected` (re-attached, not removed).
...
Returns a function with `isDynamic: true` and `fn.ssr = { kind: "lazy", props }`. Async loader with cancellation. Creates an anchor; `insertBefore`s a `loading` node (skipped under hydrate when the region holds server nodes — no flash; they stay until the fresh run replaces them). Allocates an `AbortController`; `state.lazyCleanup` (on the **anchor**) sets `isCancelled = true`, `controller.abort()`s, and removes the `loading` node plus the mounted/fallback content node (tracked in a closure local). Calls `props.loader({ signal })`, then:

- **Success** — guards on `isCancelled || !anchor.parentNode`; removes loading; clears hydrate-adopted server nodes (`clearRenderedNodes(existing, parent)`); if `component` is a function calls `component(props.props)`, else uses directly; `mountNode`s + `insertBefore` at anchor. **No nested Promise unwrapping** — loader must resolve to `ComponentFn | HellaNode`.
- **Error** — same guards; removes loading; if `props.fallback`, clears the hydrate-adopted server nodes then `resolveNode` + `insertBefore` at anchor; with no `fallback` the server content stays in place (static degradation) and the error logs. **Errors do NOT bubble to `onError`** (local fallback only).
...
- **`transitionCleanup`** (on the **anchor**) clears a pending `leaveTimer` and removes `current` (`cleanupSubtree` + `removeChild`) when the anchor is cleaned up mid-leave. The `enter` class stays on after the animation (idempotent; re-entering re-adds it).
...
- **`hydrate`** — `hydrateDynamic` → `adoptRegion`; `swapSuspenseStage` reads the staged `<template>` id from a sentinel comment, replaces the fallback with the template's resolved children, then adopts them — the no-script/HappyDOM fallback (in a browser the inline `$hs` script from `@hellajs/ssr` swaps each region as it arrives; this runs only when that script hasn't). A sentinel-without-template at `readyState "loading"` defers instead (selective hydration — see the hydrate section); at any later readyState it degrades via `stageMissing` (re-suspend client-side).
- **Fresh mount** (client, no server HTML — e.g. after client-side navigation) — evaluates the child once (`isFunction(children) ? children() : children`). A thenable suspends: render `fallback` before the anchor, await, then `insertBefore(resolveNode(resolved), anchor)`. `state.suspenseCleanup` (on the **anchor**) sets a `cancelled` flag and removes the tracked suspended nodes plus the fallback; both `.then` and `.catch` no-op on `cancelled || !anchor.parentNode` (parent removed mid-flight is safe). Sync children render directly via `resolveNode` (no fallback) and are tracked the same way.
...
- **`refObserver`** — separate from the container observer. Watches `document.body` `{ childList: true, subtree: true }`; removals feed `cleanupQueue` via the shared `collectRemovedNode` (same descendant walk as the container observer), any mutation schedules `checkMultiSelectors` via microtask. **The observer deliberately survives `$ref` resolution** — it is the removal-cleanup net for `$ref`-bound elements outside mount containers; `$ref`'s inline unregister (registry cleanup only) never disconnects it. Only `$collection.dispose()` → `unregisterMultiOp` → `cleanupRefObserver` disconnects, when `multiSelectors.size === 0` — same page-lifetime precedent as delegated event types.
...
- `reactive-dynamic-children.test.ts` — `appendToParent` Proxy forwarding for dynamic-component children; anchor-owned lifecycle switch-away (ForEach/Transition/Suspense/Lazy output removal + effect disposal, GC canary, hydrate region drain, update-hook pin).
