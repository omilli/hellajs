...
- **Fragment-root scope wiring.** The `tag: "$"` branch returns before the element wiring, so the scope rides the fragment's **first child** (`chainScopes` — the child can already carry an inner component's scope, e.g. `html\`<${B} /> tail\``); an empty fragment disposes immediately (no DOM node owns the scope). A text first child is legal: `collectRemovedNode` queues state-carrying text nodes on removal.

- **Static fast-path scope wiring.** A fully-`static` root returned by `component()` arrives as a fresh per-invocation wrapper (`{ ...result }` — the shared template node is never mutated), and a `staticDom` cache-hit clone gets `node.componentScope` re-wired onto the clone's own state (`cloneNode` carries no `ElementState`; a fragment clone wires its first child — children move out on insert, the clone itself is dead state), so every mount of the same node object (incl. `reset()` re-mounts) disposes its scope on cleanup.
