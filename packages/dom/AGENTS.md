...
| `componentScope` | Attached by `component()` (a static root is spread into a fresh node first — the shared template node is never mutated); copied to `state.componentScope` at mount, on `staticDom` clone, and on hydrate adopt. |
...
- **Static fast-path scope wiring.** A fully-`static` root returned by `component()` arrives as a fresh per-invocation wrapper (`{ ...result }` — the shared template node is never mutated), and a `staticDom` cache-hit clone gets `node.componentScope` re-wired onto the clone's own state (`cloneNode` carries no `ElementState`), so every mount of the same node object (incl. `reset()` re-mounts) disposes its scope on cleanup.
