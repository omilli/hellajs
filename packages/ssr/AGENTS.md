...
| `lib/types.d.ts` | `doc`'s option interfaces (`DocOptions` (with `body: string | ReadableStream<string>`, `mount?` selector, `data?` JSON payload)/`HeadOptions`/`MetaTag`/`LinkTag`/`ScriptTag`); wholesale-re-exported via `export type *`. |
