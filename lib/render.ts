...
export function render(
	...
	const processedVNodes = vNodes.map((vNode) =>
		processVNode(vNode, rootSelector),
	);

	// Create DOM nodes from the VNodes
	const elements = processedVNodes.map((vNode) =>
		createElement(vNode, rootSelector),
	);
...
function processVNode(
	vNode: VNode | VNodeFlatFn,
	rootSelector: string,
	parentProps?: VNodeProps<HTMLTagName>,
): VNode {
			...
			if (hasListFunctions) {
				...
				for (let i = 0; i < vNode.children.length; i++) {
					if (isObject(vNode.children[i])) {
						vNode.children[i] = processVNode(
							vNode.children[i] as VNode,
							rootSelector,
							currentProps,
						);
					}
