...
- **Manual Fetching**: Resources don't auto-fetch on creation - you must explicitly call `.fetch()` or `.request()`
- **Memory Management**: Each unique key creates a separate cache entry - consider key design for large datasets
- **Error Handling**: Always check both `error()` and `status()` states for proper error handling
- **Abort on Cleanup**: In effects or components, consider calling `.abort()` during cleanup to prevent memory leaks
- **Cache Expiration**: Cached data expires based on `cacheTime` - stale data may be served until expiration
...
const profileResource = resource(
  (id) => fetch(`/api/users/${id}`).then(r => r.json()),
  { key: () => currentUserId(), cacheTime: 300000 } // 5 min cache
);

effect(() => {
  if (currentUserId()) {
    profileResource.fetch();
  }
});
```
...
async function likePost(postId) {
  ...
  try {
    ...
    // Revert on error
    setLikeCount(c => c - 1);
    postsResource.invalidate(); // Refetch to sync
  }
}
```