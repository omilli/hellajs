...
The input automatically updates the heading as you type—no manual DOM manipulation needed.
...
**Granular Updates**: Signals enable fine-grained reactivity—only DOM nodes that depend on the changed signal update.
...
```tsx
// ❌ Unnecessary signal for static data
const title = signal('My App'); // Never changes

// ✅ Use regular variable for static data
const title = 'My App';
```
...
<details>
<summary>🔍 **Signal Internals**</summary>
...
// count → doubled → (any effects or DOM nodes)
```
