...
- **[Computed](/reference/core/computed) Values** - How to derive state automatically 
...
- **`computed(() => count() % 2 === 0)`** - Creates a derived value that updates when `count` changes
...
- **Performance** - [Computed](/reference/core/computed) values are cached and only recalculate when dependencies actually change
...
✅ **Good:** Use effects to perform side effects
```jsx
effect(() => {
...
⛔ **Bad:** Don't use effects for derived values - use computed instead
```jsx
...
- **[Signals](/reference/core/signal)** - reactive state containers that trigger automatic UI updates
- **[Computed values](/reference/core/computed)** - derived state that recalculates when dependencies change  
- **[Effects](/reference/core/effect)** - side effects that run automatically when state changes
