...
| **Events** | `on:click={func}` | **✨ `onclick={func}` - arrowless supported!** |
| **Styling** | `<style>` blocks | CSS-in-JS or classes |
| **Build Process** | Svelte compiler | Standard bundlers |
| **Templates** | Svelte syntax | JSX with [flexible bindings](/learn/concepts/templates#reactive-bindings) |
...
const Counter = () => {
  ...
  return (
    <div>
      ...
      {/* Traditional arrow function (like Svelte) */}
      <button onclick={() => count(count() + 1)}>Increment (Arrow)</button>
      
      {/* ✨ HellaJS UNIQUE: Arrowless events! */}
      <button onclick={increment}>Increment (Arrowless)</button>
      <button onclick={console.log.bind(null, 'Count:', count())}>Log Count</button>
    </div>
  );
};
```
...
- HellaJS uses explicit `signal()` functions instead of Svelte's compiler magic
- **✨ Arrowless events**: HellaJS supports `onclick={functionName}` without arrow function wrappers - a unique advantage over Svelte
- Both frameworks support fine-grained reactivity, but HellaJS is more explicit
...
const ReactiveExample = () => {
  ...
  return <p>{count()} doubled is {doubled()}</p>;   {/* Flexible patterns */}
};
```
...
**Svelte (requires arrow functions for inline operations):**
```svelte
<script>
  let name = '';
  let count = 0;
...
<form on:submit={handleSubmit}>
  ...
  <!-- Svelte needs arrow functions for simple operations -->
  <button on:click={() => console.log('clicked')}>Log</button>
  <button on:click={() => alert('hello')}>Alert</button>
</form>
```

**HellaJS (supports both arrow and arrowless patterns):**
```jsx
import { signal } from '@hellajs/core';
...
const FormExample = () => {
  ...
  return (
    <form onsubmit={handleSubmit}>
      ...
      {/* Traditional arrow functions (like Svelte) */}
      <button onclick={() => count(count() + 1)}>
        Clicked {count} times (Arrow)
      </button>
      
      {/* ✨ HellaJS advantage: Arrowless events reduce boilerplate */}
      <button onclick={incrementCount}>
        Clicked {count} times (Arrowless)
      </button>
      <button onclick={console.log.bind(null, 'clicked')}>Log (Arrowless)</button>
      <button onclick={alert.bind(null, 'hello')}>Alert (Arrowless)</button>
    </form>
  );
};
```
...
<div role="alert" class="alert alert-info text-base">
  <Icon name="ph:info" width={24} height={24} />
  <span>**Flexible Bindings**: HellaJS supports both `{count}` and `{count()}` in JSX - [both create reactive bindings](/learn/concepts/templates#reactive-bindings). Choose whichever feels natural.</span>
</div>
