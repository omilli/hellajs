...
const Counter = () => {
  ...
  return (
    <div>
      <h1>{count}</h1> {/* Flexible: both {count} and {count()} work */}
...
**Key Differences:**
- HellaJS: [Flexible JSX bindings](/learn/concepts/templates#reactive-bindings) - both `{count}` and `{count()}` work
- HellaJS: Event handlers use lowercase (`onclick` vs `onClick`)
- **✨ Arrowless events**: HellaJS supports `onclick={functionName}` without arrow function wrappers - unique advantage over Solid
...
HellaJS offers [flexible reactive bindings](/learn/concepts/templates#reactive-bindings):

```jsx
// Both patterns work in HellaJS:
<h1>{count()}</h1>   // Function call - works
<h1>{count}</h1>     // Function reference - also works

// Choose the style that feels most natural to you
```

### 5. Convert Event Handlers - Arrowless Advantage ✨

**HellaJS offers a unique advantage over Solid with arrowless event handlers:**

```jsx
// Before (Solid - requires arrow functions or setter calls)
<button onClick={() => setCount(count() + 1)}>+</button>
<button onClick={() => console.log('clicked')}>Log</button>

// After (HellaJS - multiple options)  
<button onclick={() => count(count() + 1)}>+ (Arrow)</button>

// ✨ HellaJS UNIQUE: Arrowless events!
<button onclick={increment}>+ (Arrowless)</button>
<button onclick={console.log.bind(null, 'clicked')}>Log (Arrowless)</button>
```

**Migration Tip**: Extract functions when migrating to take advantage of arrowless syntax.
...
const Counter = () => {
  ...
  return (
    <div>
      <h1>{count()}</h1>   {/* Both {count} and {count()} work */}
      
      {/* Traditional Solid-style arrow functions */}
      <button onclick={() => count(count() + 1)}>+ Arrow</button>
      <button onclick={() => count(count() - 1)}>- Arrow</button>
      
      {/* ✨ HellaJS advantage: Arrowless events */}
      <button onclick={increment}>+ Arrowless</button>
      <button onclick={decrement}>- Arrowless</button>
    </div>
  );
...
const TodoList = () => {
  ...
  const logTodo = (todo) => console.log('Todo:', todo.text);
  
  return (
    <div>
      <button onclick={() => addTodo('New Task')}>Add Todo (Arrow)</button>
      <button onclick={addTodo.bind(null, 'Quick Task')}>Add Todo (Arrowless)</button>
      
      <ul>
        {forEach(todos, todo => (
          <li key={todo.id}>
            {todo.text}
            {/* ✨ Arrowless events in list items */}
            <button onclick={logTodo.bind(null, todo)}>Log</button>
          </li>
        ))}
      </ul>
    </div>
...
### Common Gotchas
- **Flexible signal bindings**: Both `{count}` and `{count()}` work in HellaJS JSX - [use either pattern](/learn/concepts/templates#reactive-bindings)
- Event handlers are lowercase: `onclick` not `onClick`
- **✨ Arrowless advantage**: Unlike Solid, HellaJS supports `onclick={functionName}` without arrow wrappers - take advantage of this cleaner syntax!
