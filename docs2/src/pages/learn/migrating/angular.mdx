...
- **✨ Arrowless event handlers** - unique simplicity advantage
...
const Counter = () => {
  ...
  return (
    <div>
      <h1>{count()}</h1>   {/* Both {count} and {count()} work in HellaJS */}
      
      {/* Traditional arrow functions work */}
      <button onclick={() => count(count() + 1)}>+ Arrow</button>
      
      {/* ✨ HellaJS UNIQUE: Arrowless events reduce boilerplate! */}
      <button onclick={increment}>+ Arrowless</button>
      <button onclick={decrement}>- Arrowless</button>
      <button onclick={console.log.bind(null, 'Count:', count())}>Log</button>
    </div>
  );
};
```
...
- Direct signal usage instead of class properties with [flexible JSX bindings](/learn/concepts/templates#reactive-bindings)
- **✨ Arrowless event handlers** - `onclick={functionName}` instead of `(click)="methodName()"`
- Much cleaner event syntax - no need for arrow function wrappers
...
```jsx
<div>
  <h1>{title()}</h1>    {/* Both {title} and {title()} work */}
  <p>{description}</p>  {/* Flexible reactive bindings */}
  <button 
    disabled={isLoading()}
    class={isActive() ? 'active' : ''}
  >
    {buttonText}        {/* Choose your preferred pattern */}
  </button>
  <input 
    value={inputValue}
  ...
  {/* ✨ HellaJS advantage: Arrowless events for simple operations */}
  <button onclick={console.log.bind(null, 'Button clicked!')}>Log</button>
  <button onclick={alert.bind(null, 'Hello from HellaJS!')}>Alert</button>
</div>
```
...
- Event binding `(click)` becomes lowercase `onclick`
- **✨ Arrowless events**: `onclick={functionName}` instead of `(click)="methodName()"`
- [Flexible signal bindings](/learn/concepts/templates#reactive-bindings): both `{signal}` and `{signal()}` create reactive updates
- Much cleaner syntax - no need for method calls or arrow function wrappers
        ...
        <ul>
          {forEach(users.data, user => (
            <li key={user.id}>
              ...
              {/* ✨ Arrowless alternative for debugging */}
              <button onclick={console.log.bind(null, 'Selected:', user.name)}>
                Log {user.name}
              </button>
            </li>
          ))}
...
const Header = () => {
  ...
  const loginAsJohn = () => setUser({ name: 'John' });
    ...
    <header class={`header theme-${theme()}`}>
      ...
      {isLoggedIn() ? (
        <div>
          ...
          {/* ✨ Arrowless event - clean and simple */}
          <button onclick={logout}>Logout</button>
        </div>
      ) : (
        <div>
          {/* Traditional arrow function when needed */}
          <button onclick={() => setUser({ name: 'John' })}>Login (Arrow)</button>
          
          {/* ✨ Arrowless alternative */}
          <button onclick={loginAsJohn}>Login (Arrowless)</button>
        </div>
      )}
      
      {/* Both patterns work - choose your preference */}
      <button onclick={() => setTheme(theme() === 'light' ? 'dark' : 'light')}>
        Toggle Theme (Arrow)
      </button>
      <button onclick={toggleTheme}>Toggle Theme (Arrowless)</button>
    </header>
  );
};
```
...
4. **Convert event handlers (with arrowless advantage):**
   ```html
   <!-- Before: Angular's verbose syntax -->
   <button (click)="increment()">+</button>
   <button (click)="console.log('clicked')">Log</button>
   
   <!-- After: HellaJS flexible options -->
   <button onclick={() => count(count() + 1)}>+ (Arrow)</button>
   <button onclick={increment}>+ (Arrowless)</button>
   
   <!-- ✨ Unique HellaJS advantage: No wrappers needed! -->
   <button onclick={console.log.bind(null, 'clicked')}>Log (Arrowless)</button>
   ```
...
### 3. Understanding Reactive Bindings
```jsx
// ✅ Both patterns work in HellaJS and create reactive bindings
<div>{count()}</div>    // Function call - reactive
<div>{count}</div>      // Function reference - also reactive

// See the templates guide for details: /learn/concepts/templates#reactive-bindings
```
