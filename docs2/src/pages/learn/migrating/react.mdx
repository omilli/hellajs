...
### HellaJS: Direct DOM Binding + Arrowless Events
HellaJS creates direct reactive bindings between signals and DOM elements, eliminating the need for virtual DOM diffing and re-renders. Additionally, **HellaJS supports arrowless event handlers** - a unique feature that reduces boilerplate compared to other frameworks.
...
const App = () => {
  ...
  return (
    <div>
      <h1>Hello, {name}!</h1>    {/* Both {name} and {name()} work */}
      <p>Count: {count()}</p>     {/* Both {count} and {count()} work */}
      <button onclick={() => count(count() + 1)}>+</button>
      <input value={name} oninput={e => name(e.target.value)} />
      
      {/* ✨ UNIQUE: HellaJS supports arrowless events! */}
      <button onclick={console.log.bind(null, 'No arrow needed!')}>Log</button>
      <button onclick={alert.bind(null, 'Direct function call')}>Alert</button>
    </div>
  );
};
```
...
- **Flexible JSX bindings**: Use either `{count}` or `{count()}` - [both work](/learn/concepts/templates#reactive-bindings)
- **Arrowless event handlers**: Unlike React, HellaJS supports direct function calls like `onclick={console.log}` without arrow functions
...
- **✨ Arrowless events work**: `onclick={addTodo.bind(null, 'New todo')}` instead of `onClick={() => addTodo('New todo')}`
    ...
    <ul>
      {forEach(todos, todo => (
        <li key={todo.id}>
          ...
          />
          {todo.text}
          {/* ✨ Arrowless events for simple actions */}
          <button onclick={console.log.bind(null, `Todo: ${todo.text}`)}>Log</button>
        </li>
      ))}
    </ul>
  );
...
- **✨ Arrowless events in lists**: Perfect for logging, debugging, or simple operations within iterations
...
## Event Handling - Arrowless Events Advantage ✨

**One of HellaJS's most unique features is support for arrowless event handlers**, dramatically reducing boilerplate code compared to React.

**React (verbose arrow functions required):**
```jsx
function Button() {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {/* React requires arrow functions for simple operations */}
      <button onClick={() => console.log('Must use arrow')}>Log</button>
      <button onClick={() => alert('Must wrap in arrow')}>Alert</button>
...
**HellaJS (supports both patterns - much cleaner!):**
```jsx
const Button = () => {
  const handleClick = (event) => {
    console.log('Clicked:', event.target);
  ...
  return (
    <form onsubmit={handleSubmit}>
      ...
      {/* ✨ HellaJS UNIQUE ADVANTAGE: Arrowless events reduce boilerplate! */}
      <button onclick={console.log.bind(null, 'No arrow needed!')}>Log</button>
      <button onclick={alert.bind(null, 'Clean and simple')}>Alert</button>
      <button onclick={window.print}>Print</button>
      
      {/* Traditional arrow functions also work when needed */}
      <button onclick={() => console.log('Arrow when you want it')}>Arrow</button>
    </form>
  );
};
```
...
### Key Differences:
- **✨ Arrowless events**: HellaJS supports `onclick={functionName}` and `onclick={console.log}` without arrow function wrappers
- **Reduced boilerplate**: No need for `() =>` wrapper for simple function calls
- **Lowercase event names**: `onclick` instead of `onClick`
- **Both patterns supported**: Use arrow functions when you need them, skip them when you don't
...
Change event prop names from camelCase to lowercase and **take advantage of arrowless events** to reduce boilerplate. Replace `onClick={() => func()}` with `onclick={func}` where possible.
...
HellaJS supports multiple flexible patterns for reactive bindings. See the [Reactive Bindings guide](/learn/concepts/templates#reactive-bindings) for complete details.

```jsx
// ✅ Both patterns work in HellaJS - use whichever feels natural
<div>{count}</div>      // Function reference (automatic binding)
<div>{count()}</div>    // Function call (also reactive)

// ✅ For derived values, both approaches work:
<div>{count() * 2}</div>        // Direct calculation
<div>{() => count() * 2}</div>  // Arrow function
```

**Key Point**: HellaJS is flexible - you can use either `{count}` or `{count()}` in JSX. Both create reactive bindings.
...
### Event Handler Names and Arrowless Advantage
```jsx
// ❌ React - verbose arrow functions required
...
// ✅ HellaJS - arrowless events reduce boilerplate
<button onclick={console.log.bind(null, 'clicked')}>Click</button>
<button onclick={alert.bind(null, 'hello')}>Alert</button>
<button onclick={handler}>Handler</button>
```

**Key Advantage**: HellaJS reduces verbose arrow function wrappers for simple operations, making code cleaner and more readable.
