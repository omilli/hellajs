...
|-------------|-------------------|----------------|
| `ref()` / `reactive()` | `signal()` | Function-based API, [flexible JSX bindings](/learn/concepts/templates#reactive-bindings) |
| `computed()` | `computed()` | Similar API, lazy evaluation |
| `watch()` / `watchEffect()` | `effect()` | Automatic dependency tracking |
| `<template>` | JSX | JavaScript expressions, both `{signal}` and `{signal()}` work |
| `@click="func()"` | `onclick={func}` | **✨ Arrowless events supported** - no `() =>` wrappers needed |
...
const Counter = () => {
  ...
  return (
    <div>
      ...
      <h2>{doubled()}</h2>    {/* Both {doubled} and {doubled()} work */}
      <p>{user().name}</p>
      
      {/* ✨ HellaJS advantage: Arrowless events reduce boilerplate */}
      <button onclick={() => count(count() + 1)}>Traditional arrow</button>
      <button onclick={console.log.bind(null, 'Arrowless!')}>Log without arrow</button>
    </div>
  )
}
```
...
const App = () => {
  ...
  return (
    <div>
      <h1>{title}</h1>         {/* Flexible: {title} or {title()} */}
      {isVisible() && <p>{message()}</p>}
      <button onclick={increment}>Count: {count()}</button>  {/* Both patterns work */}
      <input 
        value={inputValue}      {/* Flexible binding */}
...
- HellaJS offers [flexible reactive bindings](/learn/concepts/templates#reactive-bindings) - both `{signal}` and `{signal()}` create reactive updates in JSX
- **✨ Arrowless events**: Unlike Vue's template syntax, HellaJS supports direct function references like `onclick={console.log}` without arrow function wrappers
- Choose the syntax style that feels most natural for your team
...
**Vue requires method calls or arrow functions:**
```html
<template>
  <button @click="handleClick">Click me</button>
  <button @click="increment(5)">Add 5</button>
  <button @click="() => console.log('Need arrow')">Log</button>
  <button @click="() => alert('Vue needs wrapper')">Alert</button>
  <input @input="onInput" />
</template>
```

**HellaJS supports both traditional and arrowless patterns:**
```jsx
const EventExample = () => {
  const count = signal(0)
  ...
  return (
    <div>
      <nav>
        ...
        {/* ✨ Or use arrowless events with .bind() */}
        <button onclick={navigate.bind(null, '/user/123')}>User 123</button>
      </nav>
...
const ContactForm = () => {
  ...
  return (
    <form onsubmit={handleSubmit}>
      ...
      <button type="button" onclick={console.log.bind(null, 'Form state:', form)}>Debug</button>
    </form>
  )
      name: form.name(),
      email: form.email()
