...
const TemplateExample = () => {
  ...
  return (
    <div>
      {/* ✅ Reactive binding - updates when name changes */}
      <h1>{greeting}</h1>
      
      {/* ✅ Reactive binding - updates when count changes */}
      <p>Count: {count}</p>
      
      {/* ❌ Static value - called once, never updates */}
      <p>Static: {count()}</p>
...
```jsx
// ✅ Good - only updates when specific data changes
const EfficientList = () => {
  const users = signal([
...
// ❌ Less efficient - creates new functions on every render
...
- **⚛️ [Reactivity](/learn/concepts/reactivity)** - Understanding the reactive system behind JSX
- **🧩 [Components](/learn/concepts/components)** - Building reusable components with JSX
- **🎨 [Styling](/learn/concepts/styling)** - Adding CSS and styling to your templates
- **🔧 [Event Handling](/learn/concepts/events)** - Advanced event handling patterns
