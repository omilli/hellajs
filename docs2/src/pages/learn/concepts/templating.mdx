...
title: Templating
description: Master JSX templating in HellaJS with reactive bindings and control flow.
---

HellaJS uses JSX for templating, but with a crucial difference from React: JSX expressions create **direct reactive bindings** to DOM elements, not virtual DOM nodes.
...
> **Key Insight**: Each reactive expression `{user().name}`, `{count}`, etc. creates a direct binding to a specific DOM node. When the signal changes, only that exact element updates.
...
<details>
<summary>Complex Event Handling</summary>
...
<details>
<summary>Dynamic Component Rendering</summary>
...
const DynamicContent = () => {
  const content = signal([
    { type: 'text', data: { content: 'Welcome to our site!' } },
    { type: 'image', data: { src: '/logo.png', alt: 'Logo' } },
    { type: 'button', data: { label: 'Click me!', onClick: () => alert('Clicked!') } },
    { type: 'list', data: { 
      items: [
  ...
  return (
    <div>
      {content().map((block, index) => {
        ...
        return Component ? (
          <div key={index} class={`block block-${block.type}`}>
