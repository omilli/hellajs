...
Resources provide a reactive way to handle asynchronous data with built-in loading states, error handling, and caching. They eliminate the boilerplate of managing async state manually.
...
> **Key Insight**: Resources automatically track loading, error, and success states. All these states are reactive and update your UI automatically.
...
Combine multiple resources for complex data flows:
...
const UserProfilePage = () => {
  ...
  const followers = resource(
    id => fetch(`/api/users/${id}/followers`).then(r => r.json()),
    { key: userId }
  );

  // Computed loading state
  const isLoading = computed(() => 
    user.loading() || posts.loading() || followers.loading()
  );
  ...
  return (
    <div>
      ...
      {user.data() && (
        <div>
          <h1>{user.data().name}</h1>
          <p>Followers: {followers.data()?.length || 0}</p>
          
          <div>
            <h2>Posts ({posts.data()?.length || 0})</h2>
            {posts.data()?.map(post => (
              <article key={post.id}>
                <h3>{post.title}</h3>
                <p>{post.content}</p>
...
Implement infinite scrolling with resource pagination:

```jsx
const InfinitePostList = () => {
  const page = signal(1);
  const allPosts = signal([]);
  
  const posts = resource(
    p => fetch(`/api/posts?page=${p}&limit=10`).then(r => r.json()),
    { 
      key: page,
      onSuccess: (data) => {
        if (page() === 1) {
          allPosts(data); // First page - replace
        } else {
          allPosts([...allPosts(), ...data]); // Subsequent pages - append
        }
      }
    }
  );
