...
When an effect function is async, it will re-run whenever its tracked dependencies change, canceling any previous execution.
...
Each property (_with deep nesting support_) in a store is a signal, preventing unnecessary recomputations when unrelated properties change.
...
Read signal values without establishing a dependency relationship  and triggering updates when that signal changes.
