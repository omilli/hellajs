...
title: Routing
description: Client-side routing with HellaJS Router.
---

HellaJS Router provides reactive client-side routing for single-page applications. Unlike traditional routers that use JSX-based route definitions, HellaJS uses function-based route handlers that integrate seamlessly with reactive signals.
...
Load data when routes change and show loading states:
...
router({
  routes: {
    '/user/:id': async (params) => {
      ...
      currentView(<div>Loading user...</div>);
      
      try {
        const user = await fetch(`/api/users/${params.id}`).then(r => r.json());
        currentView(<UserProfile user={user} />);
      } catch (error) {
        currentView(<div>Error loading user: {error.message}</div>);
      }
    }
  }
});
...
Handle complex data loading with dependencies:
...
Manage complex routing state across your application:

```jsx
const routeState = signal({
  currentRoute: null,
  params: {},
  query: {},
  loading: false
...
router({
  routes: {
    '/': (params, query) => {
      routeState({ currentRoute: 'home', params, query, loading: false });
      currentView(<Home />);
    },
    '/user/:id': (params, query) => {
      routeState({ currentRoute: 'user', params, query, loading: true });
...
// Other components can access route state
const Breadcrumbs = () => {
