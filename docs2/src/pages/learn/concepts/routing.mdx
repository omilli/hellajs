...
HellaJS Router provides reactive client-side routing that integrates seamlessly with the signal-based reactivity system. Routes are defined as simple functions that execute when paths match, enabling reactive navigation patterns without complex lifecycle management.

<div role="alert" class="alert alert-info alert-soft text-base">
  <Icon name="ph:info" width={24} height={24} />
  <span>The router package uses signals internally, making all routing state reactive to your components.</span>
</div>

## Basic Setup

The router accepts a configuration object defining routes as pattern-to-handler mappings. Route handlers execute immediately when their pattern matches, typically updating a reactive signal that drives UI changes.
...
Dynamic route segments are captured using the `:parameter` syntax. Parameters are automatically extracted and passed as the first argument to route handlers.
...
Query parameters are automatically parsed from the URL and passed as the second argument to route handlers, enabling reactive responses to URL state changes.
...
The `route` signal provides reactive access to the current route information, including the matched handler, extracted parameters, query string, and current path. This signal updates automatically whenever navigation occurs.
...
Understanding the router's internal architecture reveals how it achieves reactive routing with minimal overhead while integrating seamlessly with HellaJS's signal system.
...
This architecture delivers powerful routing capabilities while maintaining the lightweight, reactive philosophy of the HellaJS framework, ensuring that routing state changes propagate efficiently through your entire application without manual subscription management or complex lifecycle coordination.