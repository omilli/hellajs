...
[Signals](/reference/core/signal) are just functions. To bind a signal to an element, pass the reference directly instead of calling it.
...
You don't need computed values for simple derived properties. Just use a function that returns the desired value.
...
Dynamic string interpolation works seamlessly with reactive values. Use function references to create dynamic text content.
...
Functions that return JSX (and ultimately VNodes). They support reactive data flow through signals.
...
### Derived Functions vs Computed

Inside components, you can create derived functions without using [`computed()`](/reference/core/computed). This is simpler for basic derivations, while [`computed()`](/reference/core/computed) provides memoization for expensive operations.
...
const TodoStats = () => {
  ...
  // Use [computed()](/reference/core/computed) only when you need memoization for expensive operations
...
<div role="alert" class="alert alert-error text-base">
  <Icon name="ph:warning" width={24} height={24} />
  <span>Control flow also uses function references instead of direct values.</span>
</div>


### Conditional Rendering

Familiar conditional rendering, using function references to ensure reactivity.

✅ Reactive: ` <p>Count is: {() => count() % 2 === 0 ? 'even' : 'odd'}</p>`

⛔ Static: `  <p>Count is: {count() % 2 === 0 ? 'even' : 'odd'}</p>`
