...
Signals are just functions. To bind a signal to an element, pass the reference directly instead of calling it.
...
Dynamic string interpolation works seamlessly with reactive values. Uuse function references to create dynamic text content.
...
Group elements without a wrapper element. Great for when a component's structure does't align with semantic HTML.
...
Use [untracked](/reference/core/untracked) to read signals without creating dependencies.
