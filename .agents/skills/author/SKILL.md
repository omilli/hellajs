...
**Rules/style guide specifically**: open with a Contents decision index (decision → section rows — the navigation entry point, `docs.md` the canonical form), then three layers — decision trees at top (traverse to a decision), prose rules in middle, verification checklist at end (the audit floor; the matching `audit-*` skill ticks it instead of reconstructing rules from prose). Open the rules with a one-line **Decision Precedence** (ranked tradeoffs) so conflicts resolve consistently. `guides/` is the live example of this shape — match it.
