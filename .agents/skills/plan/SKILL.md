...
name: plan
description: >
  Lay out a plan — one or more tracked, verifiable contracts (files to touch, binary Definitions of Done, blast radius, hard inter-file deps) — before executing non-trivial work. Use when work spans multiple steps or files, when you want a reviewable spec before implementation, or when handed evidence maps from discovery (idea/audit/feature). Produces one file per independently-shippable unit. Use ONLY for non-trivial work; a one-line fix skips planning entirely.
---
...
- **Change/delta** — for Surface:yes, the exact signature/shape change + one runnable usage example (if you can't write the call, the design is wrong; the example seeds the Docs task). A delta clause citing repo style or convention carries the measurement that produced it (the `rg` over the package) — an unmeasured style claim inverts silently. A delta premised on type-system behavior probes the usage example itself through a throwaway `tsc` run before the contract is written — probed resolutions and a separately-recorded usage can contradict each other (resolutions the usage cannot compile against), and only compiling the usage catches it.
...
5. **Sweep DoDs scope to their artifact class.** A repo-wide sweep DoD (`rg <pattern>`) names the artifact class it polices and excludes `plans/**` — the contract quotes the strings it retargets, so the bare pattern matches the plan's own text and can never pass as written. The same collision arrives via sibling tasks: a Docs task required to name the retargeted symbol (file-map rows, doc-sync mentions) makes the bare pattern match a file the plan itself edits — scope the sweep to the artifact class it polices (`-g '*.ts'` for code annotations), never the whole tree. The file-scoped variant: an rg DoD scoped to named files still sweeps the whole file, where legitimate constructs of the same bare kind can sit outside the delta's scope — probe the pattern's match set on the target files before writing the contract, and key it to a structural discriminator of the banned artifact class (the poll loop's `break;` guard, not any `delay(10)`), because a baseline match set exceeding the delta makes the DoD unsatisfiable as written no matter how faithfully the worker executes. The removal variant: a check asserting a string's absence must not search a fixed string that survives inside the unit's own mandated replacement (`$subscribe(key, cb)` contains `subscribe(key, cb)`, so a `-F` removal check can never exit 1 once the delta lands) — key the absence check to a guarded pattern (a char class excluding `$`, or a `| rg -v '\$'` stage) and verify it against the post-delta text.
6. **DoD glosses restate the delta, never widen it.** A DoD's paraphrase of the target state ("every `css(`/`style(` call … passes a label options bag") must be satisfiable by the delta's verbatim output — a gloss demanding more than the delta specifies (label bags when the delta's specified call is unlabeled `style({ color })`) is untickable as written and forces the worker to adjudicate the plan against itself mid-flight. Derive the gloss from the delta's specified shape, term by term.
7. **Numeric DoD thresholds verify their levers' arithmetic.** When a DoD sets a measurable cap (block ≤ N lines, bundle ≤ M kb) and the delta names specific levers, sum the named levers' net against the measured baseline before writing the contract — levers netting short of the threshold hand the worker a plan gap to adjudicate mid-flight. Either name every lever needed to close the gap or set the threshold to what the named levers reach.
...
Write one file per unit to `plans/<package>/<category>/<topic>/<unit>.md` (categories observed: `code`, `docs`, `misc`, `config`; audit-category sets live under `plans/<pkg>/audit/`, written by `bun audits`), frontmatter `depends_on: [stems]` (inline array — block lists are unparseable) if hard deps exist. For N>1 also write `index.md`: `# [ ] Plan set: <topic>` aggregate, shared scope, sibling links with one-line descriptions + hard deps; state the dep graph. On approval, hand to `worker` — don't execute yourself unless it was escape-hatch small.
