...
- **Surplus delta** — worktree-diff files beyond your component's units' Files lists (sibling-unit work landed in a mis-partitioned venue; a `depends_on` form `readDependsOn` can't parse is the usual root) → operator fork: absorb as one commit / split into per-unit commits / stop and fix the derivation input. Never silently absorb or clean it away. One artifact class is mechanical, not a fork: `bun.lock` churn from the seed's `bun install` when the main-tree lock is stale against a committed package.json — a unit that legitimately needs a lockfile delta lists it in Files, so surplus lock churn is never contract work. Restore the worktree's `bun.lock` to baseline content before `worktree.mjs commit`, and report the staleness for a user-side lock refresh.

- **Staged-set hygiene** — before any main-tree `git commit` (fix rounds, the memory amend), verify the staged set is exactly yours (`git diff --cached --stat`): the operator shares the tree, and concurrently staged in-flight work rides a blind commit — reset and restage rather than sweeping it.

## Step 2 — Commit, cherry-pick, resolve

1. Derive the message from the unit contract: `<type>(<scope>): <imperative subject from the unit title>`. Scope follows the release decision (§Core rules): `feat`/`fix` scope = the published workspace the API delta touches — the set folder only when they coincide (`plans/docs/…` carrying a `packages/dom` type delta commits `feat(dom)`, never `feat(docs)`); non-bumping types scope to the set's package. Type is release-gated (§Core rules): `feat`/`fix` ONLY when the delta lands in a published package's user-facing API; every other delta (docs, scripts, agent-config, tests, tooling, internal structure) takes the matching non-bumping type regardless of novelty. A breaking unit adds `!` + a `BREAKING CHANGE:` footer. The commit-msg hook enforces the format — satisfy it, one body line citing the unit's plan file. Multi-unit components land one commit: mixed bump/non-bump → the user-facing API delta's type + its package as scope, else the dominant non-bumping type. Never create a changeset; releases are user-only.
