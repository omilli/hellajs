...
**Provision.** `bun .agents/skills/worker/scripts/worktree.mjs new <slug> --plans <set-folder>` (base `v2` default). Cuts clean from the base, carries exactly the plan-set folder (never committed, so a cut has none) and the uncommitted `memory/` delta, records the post-seed baseline; `diff`/`commit` are baseline-relative, so unchanged carries are invisible. The seed's internal `bun install` can rewrite `bun.lock` when the committed lock is stale against committed `package.json`s; that rewrite is install drift, never a component's delta — restore it (`git checkout HEAD -- bun.lock`) so the diff stays pure; `node_modules` is already installed, so guards run on unaffected.
