...
- **Read the KB first on every task.** Before opening any target source/test/doc: `memory.ts query <term>...` with the target's module names and exposed symbols (ranked titles + descriptions, top 10; `--all` widens). Open a full entry only when its description is load-bearing for this task; `rg <pattern> memory/index.md` when a regex or alternation probe is needed. Run `memory.ts stale`; re-verify anything listed before trusting it.
- **Query the KB on failure, not just up front.** A failed check or a retry: `memory.ts query` the error's distinctive tokens (`happydom location`, `uncovered closing brace`) before forming a hypothesis — the trigger vocabulary is failure-shaped for exactly this moment.
