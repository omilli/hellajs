...
import { getAllPackages, getPackagesWithChangesets, getPackagePath } from "./utils/packages.js";
import { getPackageVersion, updatePeerDependencies, parseChangesetBumps, bumpVersion } from "./utils/versions.js";
import { createResult, addPreviewOperation } from "./utils/validation.js";
  ...
  for (const packageName of packagesWithChanges) {
    ...
    if (packagePath) {
      ...
      if (currentVersion) {
        ...
        logger.info(`[DRY-RUN] ${packageName}: ${currentVersion} → ${newVersion} (${bumpType})`);
      }
    }
  }
...
function displayPreviewSummary(packageVersions, previewOperations) {
    ...
    for (let i = 0; i < previewOperations.length; i++) {
      ...
      if (op.details.changes) {
        for (const change of op.details.changes) {
          logger.info(`   - ${change.type}: ${change.dependency} (${change.from} → ${change.to})`);
        }
      }
    }
    logger.info(`${previewOperations.length + 2}. Run 'changeset publish' to publish packages to NPM`);
  } else {
  ...
  logger.info("To perform the actual release, run this script without the --dry-run flag.");
  logger.info("=".repeat(60));
}
...
function validateForPublishing() {
  ...
  for (const pkg of packages) {
    ...
    if (pkg.packageJson.peerDependencies) {
      for (const [depName, depVersion] of Object.entries(pkg.packageJson.peerDependencies)) {
        if (depName === '@hellajs/core' && !depVersion.startsWith('^')) {
          errors.push(`Invalid core peer dependency version in ${pkg.name}: ${depVersion}`);
        }
      }
    }
  }
...
async function publish(dryRun) {
  ...
  if (packageVersions.has('@hellajs/core')) {
    ...
    logger.info(`${updateMessage} @hellajs/core peer dependencies to v${coreVersion}...`);
    
    const coreDepPackages = ['@hellajs/dom', '@hellajs/store', '@hellajs/resource', '@hellajs/router'];
    for (const pkgName of coreDepPackages) {
      const pkgPath = getPackagePath(pkgName);
      if (pkgPath) {
        const updateResult = updatePeerDependencies(pkgPath, { '@hellajs/core': coreVersion }, dryRun);
        if (dryRun && updateResult.changed) {
          addPreviewOperation(result, 'update_dependencies', `Update dependencies in ${pkgName.replace('@hellajs/', '')}`, {
  ...
  if (packageVersions.has('babel-plugin-hellajs')) {
    ...
    const updateMessage = dryRun ? "[DRY-RUN] Would update" : "Updating";
    logger.info(`${updateMessage} babel-plugin-hellajs dependencies to v${babelVersion}...`);
    
    const pluginDepPackages = ['vite-plugin-hellajs', 'rollup-plugin-hellajs'];
    for (const pkgName of pluginDepPackages) {
      const pkgPath = getPackagePath(pkgName);
      if (pkgPath) {
        const updateResult = updatePeerDependencies(pkgPath, { 'babel-plugin-hellajs': babelVersion }, dryRun);
        if (dryRun && updateResult.changed) {
          addPreviewOperation(result, 'update_dependencies', `Update dependencies in ${pkgName}`, {
            changes: updateResult.changes
          });
        }
      }
