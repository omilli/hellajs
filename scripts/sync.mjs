...
import { ensureDir, fileExists, logger } from "./utils/common.js";
...
function buildAgentHeader(metadata, examples) {
	...
	const { text: cleanDesc, examples: extractedExamples } =
...
function processAgentContent(content) {
	...
	const filteredFrontmatter = filterFrontmatter(frontmatterLines, [
		"name",
		"description",
	]);
	const examplesMatch = markdownContent.match(
		/## Examples\s*([\s\S]*?)\s*## Role/,
	);
	const examples = examplesMatch ? examplesMatch[1].trim() : "";
	let cleanMarkdown = markdownContent.replace(
		/## Examples[\s\S]*?## Role/,
		"## Role",
	);
...
async function execute() {
	...
	stats.agentsProcessed =
		agentResults.status === "fulfilled" ? agentResults.value.length : 0;
	...
	return {
		...
		claudeResults:
			claudeResults.status === "fulfilled" ? claudeResults.value : [],
