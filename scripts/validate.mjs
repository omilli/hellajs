...
import { createValidationResult, addError, addWarning } from "./utils/validation.js";
...
function validateDependencyConsistency(result) {
  ...
  for (const pkg of packages) {
    const deps = [
      ...(pkg.packageJson.dependencies ? Object.entries(pkg.packageJson.dependencies) : []),
      ...(pkg.packageJson.peerDependencies ? Object.entries(pkg.packageJson.peerDependencies) : []),
      ...(pkg.packageJson.devDependencies ? Object.entries(pkg.packageJson.devDependencies) : [])
    ];
  ...
  for (const [depName, versions] of allDependencies) {
    ...
    if (uniqueVersions.size > 1) {
      addWarning(result, `Inconsistent versions for ${depName}: ${Array.from(versions).join(', ')}`);
    }
  }
  ...
  if (corePackage) {
    const coreDependentPackages = packages.filter(pkg => 
    ...
    for (const pkg of coreDependentPackages) {
      ...
      if (!coreVersion.startsWith('^')) {
        addError(result, `Invalid core peer dependency version in ${pkg.name}: ${coreVersion} (should start with ^)`);
      }
    }
  }
...
function validatePackageStructure(result) {
  ...
  for (const pkg of packages) {
    ...
    if (pkg.packageJson.exports) {
      if (!pkg.packageJson.exports['.']) {
        addError(result, `Invalid exports field in ${pkg.name}: missing '.' entry`);
      }
    }

    // Check for dist files (packages) or built files (plugins)
    const distPath = path.join(pkg.path, 'dist');
    const hasBuiltFiles = pkg.type === 'plugin' ? 
      fs.existsSync(path.join(pkg.path, 'index.mjs')) :
      fs.existsSync(distPath);
      
    if (!hasBuiltFiles) {
      addWarning(result, `No ${pkg.type === 'plugin' ? 'built files' : 'dist directory'} found for ${pkg.name} (may need to build)`);
    }
  }
...
function validatePublishingReadiness(result) {
  ...
  for (const pkg of packages) {
    ...
    if (!hasBuiltFiles) {
      addError(result, `Package ${pkg.name} is missing ${pkg.type === 'plugin' ? 'built files' : 'dist directory'} - run build first`);
      continue;
    }
    ...
    if (pkg.packageJson.main) {
      ...
      if (!fs.existsSync(mainFile)) {
        addError(result, `Package ${pkg.name} main file does not exist: ${pkg.packageJson.main}`);
      }
    }
    ...
    if (pkg.packageJson.exports && pkg.packageJson.exports['.']) {
      ...
      if (typeof exportEntry === 'string') {
        ...
        if (!fs.existsSync(exportFile)) {
          addError(result, `Package ${pkg.name} export file does not exist: ${exportEntry}`);
        }
      } else if (exportEntry.import) {
        const importFile = path.join(pkg.path, exportEntry.import);
        if (!fs.existsSync(importFile)) {
          addError(result, `Package ${pkg.name} import file does not exist: ${exportEntry.import}`);
        }
      }
    }
  }
