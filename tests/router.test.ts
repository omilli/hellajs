...
describe("router", () => {
  ...
  it("matches dynamic params", () => {
    router({
      "/user/:id": ({ id }: { id: string }) => { container.textContent = `User ${id}`; }
    });
    navigate("/user/42");
  ...
  it("handles query params", () => {
    router({
      "/search": (_: unknown, query?: { q: string }) => { container.textContent = `Query: ${query?.q}`; }
    });
  ...
  it("calls route hooks with params", () => {
    ...
    router({
      "/user/:id": {
        before: ({ id }: { id: string }) => { beforeParam = id; },
        handler: ({ id }: { id: string }) => { container.textContent = id; },
        after: ({ id }: { id: string }) => { afterParam = id; }
      }
    });
    navigate("/user/99");
  ...
  it("removes unmatched params from path", () => {
    router({
      "/foo/:id": ({ id }: { id: string }) => { container.textContent = id; }
    });
    navigate("/foo/:id", {}, {});
