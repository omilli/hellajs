...
describe("store", () => {
  test("create a user profile and update its properties", () => {
  ...
  test("partially update a product's details", () => {
    const product = store({
      id: "prod-123",
      name: "Laptop",
      details: {
  ...
  test("replace the entire state of a settings object using .set()", () => {
  ...
  test("provide a reactive computed snapshot of the current state", () => {
  ...
  test("return a plain object snapshot of the current state", () => {
    const cart = store({
      items: [{ id: 1, name: "Apple" }],
      total: 1.00
    });
  ...
  test("clean up all signals and effects associated with the store", () => {
    const session = store({
      token: "abc-123",
      user: { id: 1 }
    });
  ...
  test("update a deeply nested property in a configuration object", () => {
    const config = store({
      api: {
        baseUrl: "/api",
        endpoints: {
  ...
  test("call cleanup on nested store objects", () => {
  ...
  test("recursively clean up deeply nested plain objects", () => {
...
test("prevent all properties from being updated when readonly is true", () => {
...
test("only prevent specified properties from being updated", () => {
  const book = store({
...
test("allows nested property updates unless parent is readonly", () => {
  const user = store(
    {
      id: 1,
      profile: {
