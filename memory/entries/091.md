...
description: "babel's parser layer produces parts arrays for mixed attribute values (class=\"btn ${x}\"), but the builder folds them into binary `+` concatenation (builders/ast.mjs), so runtime html`` must concatenate cloned parts in cloneWithValues' props path (concatParts) for byte-for-byte parity; renderProp's array space-join never sees mixed parts. Judging the compiled path from the parser alone (parsers/attributes.mjs) yields the false parity model."
