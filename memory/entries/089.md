...
description: checkTutorialParity (scripts/doc-structure.ts) gates on /^## Complete Code/m, so the byte-compare now fires for every tutorial; single-file apps (no `### `src/...`` headings + exactly one src file after the vite-env.d.ts exemption) are covered by a fallback requiring the Complete-Code section's lone fence to byte-match that file (same rstrip + `// src/` breadcrumb rules). `bun lint:structure` is the parity oracle; 087's manual-replica workaround is retired — and was itself lossy (it missed blog/src/pages/UserProfile.tsx's trailing blank line, the sole example-src EOF anomaly repo-wide, which the live gate caught on first run).
