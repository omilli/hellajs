...
- `packages/router/tests/ssr.test.ts`: 5 scenarios (synchronous resolution; `url` overrides `window.location`; path params; query string; handler fires once during `router()`). `bun coverage router` exits 0 (164 pass, 100% lines).
- `packages/router/docs/patterns/ssr.mdx`: Bun.serve recipes for `ssr`/`ssrAsync`/`ssrStream` + one shared `hydrate` + pure-handler & concurrency idioms; wrapper `docs/src/pages/learn/patterns/routing-ssr.mdx`.
