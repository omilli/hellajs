...
description: HappyDOM quirk — pathname is the literal 'blank' and replaceState({}, '', '/') cannot change it, so sync router() init fires notFound during router(); mockClear() after or assert state, not counts.
