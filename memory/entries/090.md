...
description: "A `*/` sequence inside a `/** */` block comment (e.g. the glob `docs/**/*.mdx`) terminates the comment early, and the parser then reports a confusing `Unexpected *` at the token after the close — so scripts/ JSDoc spells scan scopes in prose."
