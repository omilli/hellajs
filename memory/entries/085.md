...
description: "cssText() joins the pre-split injectedMap keys, so an exact-form collector assert stays green even when registerText's rule split truncates a rule or drops siblings — anchor split behavior on sheet cssRules counts, never on the collector."
tags: [testing, css]
