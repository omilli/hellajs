<hellajs-agent>
  ...
  - **Guides are inviolable.** Source, tests, and docs follow the matching guide (`guides/code.md`, `guides/tests.md`, `guides/docs.md`). A conflict is never silently worked around — surface a **guide-update proposal** (guide + rule quoted + conflict + proposed edit with reasoning); the user accepts, rejects, or defers. Silent deviation is how uniformity dies.
