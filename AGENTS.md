<hellajs-agent>
  ...
  | worker | `bun worker <set-folder> [--wt=single\|split] [--model=…] [--thinking=…]` | Fresh `pi --mode rpc` instance per unticked plan unit, executing inside component worktrees (`--wt=split`: one per dependency-connected component). Never merges — completed components stand for `bun merge <set>`, the single human checkpoint. Ticks are read from worktree copies; main-tree copies stay `[ ]` until merge (success, not failure). Gates: retry / deliver-incomplete / abandon / halt. Worktree mechanics: `worker` skill. |
  ...
  | merge | `bun merge <set-folder> [--model=…] [--thinking=…] [--dry-run]` | Fresh instance per outstanding component worktree, executing the `merge` skill contract: mechanical queue derivation, completeness pre-check, per-component progress gate, orchestrator-owned index flip + union gate (`bun coverage <pkg>` from the set's scope or its runtime-delta packages, `bun lint` fallback; plugin exception), one fix instance per red round. `--dry-run` prints the queue. The single human checkpoint. |
  | audits | `bun audits <package> [--section=code\|tests\|docs] [--model=…] [--thinking=…] [--dry-run]` | Fresh instance per package section (fixed code → tests → docs order), each running the matching `audit-*` skill then authoring its own findings plan set under `plans/<pkg>/audit/<section>/` (three separate sets, executable via `bun worker`). Findings set present → next section; neither a set nor a clean statement → operator gate (retry / accept-clean / skip / halt). Accepts `core` or `packages/core`. `--dry-run` prints derived sections + set dirs without spawning. |
  | memory | `bun memory [--all] [--days=<n>] [--limit=<n>] [--model=…] [--thinking=…] [--dry-run]` | Knowledge-base verification runner: one fresh instance per stale memory entry (default 180d, oldest first; `--all` widens to every active entry, `--days` overrides the cutoff, `--limit` caps the queue), each running the memory skill's Step 5 verify on one entry — still true → bump `last_confirmed` + rebuild, false → author corrected concept + supersede, blocked → operator gate (retry / accept / skip / halt). The runner never verifies and never writes KB content — instances do; the outcome signal is the entry file's before/after state. `--dry-run` prints the queue. |
  | remote | `bun remote [--port=<n>] [--probe] [--model=<provider/id[:thinking]>]` | Phone-control daemon over the tailnet: serves the web panel (`scripts/remote/panel/`), authenticated WS (`hello` token frame) for run supervision (spawn, live output, writable stdin, kill), curated launcher (plan sets, presets, custom command) with same-set and merge concurrency guards, ntfy push pings on daemon start and run exits; runner gates + dialogs rendered as panel cards (first answer wins); standalone pi chat sessions from the panel (daemon-owned `pi --mode rpc` children: prompt, streamed events, steer, abort, tool approvals answered as panel cards; `--model` is the session default). `--probe` runs the full-chain self-test. Binds `127.0.0.1`; `tailscale serve --bg --tcp=8798 <port>` raw-TCP-forwards it (phone opens `http://$(tailscale ip -4):8798`: no DNS, no Host matching; `--bg <port>` HTTPS at the MagicDNS name needs outbound ACME egress); banner documents the setup. |
  | doc-snippets | `bun doc-snippets` | Audit tool (NOT a guard, `scripts/doc-snippets.ts`): typechecks every package-doc code block; strict tier gates the exit code, tutorials report informationally. Emits into and resets gitignored `.doc-snippets/` — never stage scratch files there. |
  ...
  - `docs/` — Astro docs site, importing package docs from `packages/*/docs/`. **A Docs task spans the full site surface**: `learn/concepts/` + `learn/patterns/` + `learn/tutorials/` wrapper pages, `reference/{pkg}/` API wrappers, the `components/` section (in-page demos over CLI-vendored components: `docs/src/components/ui/` holds the files `add` writes — generated, never hand-edit, sync via `bun bundle ui && bun packages/ui/bin/hellajs-ui.js add button input card dialog tabs theme --dir docs --overwrite`; each `docs/src/pages/components/<name>.astro` is self-contained — an inline `<script>` composes the vendored components through the `<${Component}>` embedded-tag syntax (close form `</${Component}>`) and `mount`s into the page's `#demo` frame (no demo modules, no `Demo.astro`, no ssr, no hydrate, no site renderer, no compile step); `docs/package.json` declares the `@hellajs/*` peers, satisfying `checkPeers`; `@layer hella;` is declared in `docs/src/global.css` after tailwind's layers so registry rules beat preflight/daisyUI; a `packages/ui/tests` drift guard fails when a vendored file diverges from fresh `add` output), `nav.ts` registration, and the enumeration indexes. A user-facing feature needs: a concept doc, a pattern doc when copy-paste recipes apply, `nav.ts` registration, and every enumeration listing it updated. Before scoping a Docs task, grep the site for prose claims the change falsifies (e.g. an "X not supported" alert).
